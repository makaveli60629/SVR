(() => {
  ...
  const CACHE_EPOCH='phase411-tournament-bot-action-readability';
  const CURRENT_PHASE='phase411';
  const QUEST_PHASE='phase396';
  const SITE_PHASE='phase392';
  const AVATAR_PHASE='phase392';
  const ANDROID_PHASE='phase411';
  ...
  async function refreshRuntimeCaches(){
    ...
    }catch(error){console.warn('SVR Phase 411 cache recovery could not complete.',error)}
  }
  ...
  function paintPhaseBadge(){if(document.getElementById('svr-phase-live-badge'))return;const badge=document.createElement('div');badge.id='svr-phase-live-badge';badge.textContent='● PHASE 411 MOBILE / TOURNAMENT TEST';badge.setAttribute('aria-label','SVR Poker Phase 411 mobile tournament, bot strategy and action-readability test');Object.assign(badge.style,{position:'fixed',top:'12px',right:'12px',zIndex:'2147483647',padding:'7px 11px',border:'1px solid #8dffb4',borderRadius:'999px',background:'rgba(0,12,18,.92)',color:'#8dffb4',font:'800 11px/1.1 system-ui,Arial,sans-serif',letterSpacing:'.08em',boxShadow:'0 0 24px rgba(141,255,180,.2)'});document.body.appendChild(badge);document.body.dataset.deployPhase=CURRENT_PHASE}
  function ensurePlatformDetector(){if(window.SVR_PLATFORM_DEVICE||document.querySelector('script[data-svr-platform]'))return;const script=document.createElement('script');script.src='/platform-device.js?v=phase411';script.dataset.svrPlatform='1';document.head.appendChild(script)}
  ...
  function applyPlatformLaunch(){
    ...
    else if(d.ios){button.innerHTML='Play iPhone<span class="svr-test-ready">IPHONE GAME • NOT VR • TEST READY</span>';button.href='./game/iphone.html?v=phase411&source=launch-device';notice.innerHTML='<strong>iPhone / iPad detected:</strong> Phase 411 phone/tablet game • protected human input • readable player actions • independent bot strategy • 100-player local tournament test • profiles and mic tools. For VR, have your Quest/Oculus headset ready.'}
    else if(d.android){button.innerHTML='Play Android<span class="svr-test-ready">ANDROID GAME • NOT VR • APP TEST READY</span>';button.href='./game/android.html?channel=stable&v=phase411&source=launch-device';notice.innerHTML='<strong>Android detected:</strong> Phase 411 phone/tablet game • protected poker engine • large action feedback • independent bot strategy • one burn pile • 100-player local tournament test. Android APK RC2 remains available from the game menu. For virtual reality, have your Quest/Oculus headset ready.'}
  ...
  function profilePortrait(){if(!/\/site\/profile\.html$/i.test(location.pathname)||document.querySelector('script[data-phase405-portrait]'))return;const s=document.createElement('script');s.type='module';s.src='/site/js/phase405-profile-portrait.js?v=phase411';s.dataset.phase405Portrait='1';document.body.appendChild(s)}

/* PRESERVED PREDECESSOR PUBLIC AUDIT MARKERS (non-runtime): PHASE 407 MOBILE FIT / PACING | PHASE 405 PLATFORM / MOBILE | PHASE 411 PUBLIC MOBILE BADGE | PHASE 409 PUBLIC / PLAYER TURN | phase407-public-status.js */
(() => {
  const ADMIN_KEY='svr_admin_presence';
  const ADMIN_PAYLOAD_KEY='svr_admin_presence_payload';
  const ADMIN_SESSION_KEY='SVR_ADMIN_SESSION';
  const MESSAGE_KEY='svr_public_messages';
  const CACHE_EPOCH_KEY='svr_public_cache_epoch';
  const CACHE_EPOCH='phase418-mobile-admin-profile-account-polish';
  const CURRENT_PHASE='phase418';
  const QUEST_PHASE='phase396';
  const SITE_PHASE='phase418';
  const AVATAR_PHASE='phase418';
  const ANDROID_PHASE='phase418';

  function safeJson(raw){try{return JSON.parse(raw||'null')}catch{return null}}
  function freshPresencePayload(){const p=safeJson(localStorage.getItem(ADMIN_PAYLOAD_KEY));if(!p||!/^(online|offline)$/i.test(String(p.state||'')))return null;const expires=Date.parse(p.expiresAt||''),updated=Date.parse(p.updatedAt||'');const fresh=(Number.isFinite(expires)&&Date.now()<expires)||(Number.isFinite(updated)&&Date.now()-updated<8*60*60*1000);return fresh?p:null}
  function validAdminSession(){const s=safeJson(localStorage.getItem(ADMIN_SESSION_KEY));if(!s||s.loggedIn===false)return null;const expires=Number(s.expiresAt||0);if(expires&&expires<Date.now())return null;return /owner|admin/i.test(String(s.role||''))?s:null}
  async function refreshRuntimeCaches(){
    ...
    }catch(error){console.warn('SVR Phase 418 cache recovery could not complete.',error)}
  }
  function getAdminState(){
    const qs=new URLSearchParams(location.search),override=qs.get('admin');
    if(override==='online'||override==='offline'){localStorage.setItem(ADMIN_KEY,override);return override}
    const payload=freshPresencePayload();if(payload)return payload.state.toLowerCase();
    if(validAdminSession())return'online';
    const raw=localStorage.getItem(ADMIN_KEY);return raw==='online'?'online':'offline'
  }
  function paintAdminState(){const current=getAdminState();document.querySelectorAll('.admin-status').forEach(element=>{element.dataset.state=current;element.classList.toggle('online',current==='online');element.classList.toggle('offline',current!=='online');element.textContent=current==='online'?'● Admin Online':'● Admin Offline'});window.SVR_PUBLIC_ADMIN_STATE={state:current,online:current==='online',build:'PHASE-418-PUBLIC-ADMIN-AI-FALLBACK-LOCK',checkedAt:new Date().toISOString()};window.dispatchEvent(new CustomEvent('svr:public-admin-state',{detail:window.SVR_PUBLIC_ADMIN_STATE}))}
  function paintPhaseBadge(){if(document.getElementById('svr-phase-live-badge'))return;const badge=document.createElement('div');badge.id='svr-phase-live-badge';badge.textContent='● PHASE 418 MOBILE / PROFILE TEST';badge.setAttribute('aria-label','SVR Poker Phase 418 mobile, profile and account-access test');Object.assign(badge.style,{position:'fixed',top:'12px',right:'12px',zIndex:'2147483647',padding:'7px 11px',border:'1px solid #8dffb4',borderRadius:'999px',background:'rgba(0,12,18,.92)',color:'#8dffb4',font:'800 11px/1.1 system-ui,Arial,sans-serif',letterSpacing:'.08em',boxShadow:'0 0 24px rgba(141,255,180,.2)'});document.body.appendChild(badge);document.body.dataset.deployPhase=CURRENT_PHASE}
  function ensurePlatformDetector(){if(window.SVR_PLATFORM_DEVICE||document.querySelector('script[data-svr-platform]'))return;const script=document.createElement('script');script.src='/platform-device.js?v=phase417';script.dataset.svrPlatform='1';document.head.appendChild(script)}
  function ensurePhase409PublicStatus(){if(document.querySelector('script[data-phase409-public-status]'))return;const script=document.createElement('script');script.src='/phase409-public-status.js?v=phase418';script.dataset.phase409PublicStatus='1';document.body.appendChild(script)}
  ...
  function applyPlatformLaunch(){
    ...
    else if(d.ios){button.innerHTML='Play iPhone<span class="svr-test-ready">IPHONE GAME • NOT VR • TEST READY</span>';button.href='./game/iphone.html?v=phase418&source=launch-device';notice.innerHTML='<strong>iPhone / iPad detected:</strong> Phase 418 phone/tablet game • protected poker • human-turn authority • profiles • account access • mic/VOX tools. For VR, use Quest/Oculus.'}
    else if(d.android){button.innerHTML='Play Android<span class="svr-test-ready">ANDROID GAME • NOT VR • TEST READY</span>';button.href='./game/android.html?channel=stable&v=phase418&source=launch-device';notice.innerHTML='<strong>Android detected:</strong> Phase 418 phone/tablet game • protected poker engine • readable bankrolls • one burn pile • visible mic/account tools. APK RC2 remains optional. For VR, use Quest/Oculus.'}
  ...
  function profilePortrait(){if(!/\/site\/profile\.html$/i.test(location.pathname)||document.querySelector('script[data-phase405-portrait]'))return;const s=document.createElement('script');s.type='module';s.src='/site/js/phase405-profile-portrait.js?v=phase418';s.dataset.phase405Portrait='1';document.body.appendChild(s)}
  ...
  window.addEventListener('svr:platform-device',applyPlatformLaunch);window.addEventListener('svr-admin-presence-change',paintAdminState);window.addEventListener('storage',event=>{if([ADMIN_KEY,ADMIN_PAYLOAD_KEY,ADMIN_SESSION_KEY].includes(event.key))paintAdminState()});requestAnimationFrame(()=>setTimeout(applyPlatformLaunch,0));setTimeout(applyPlatformLaunch,350);setTimeout(normalizePublicLinks,400);setInterval(paintAdminState,1500);
})();
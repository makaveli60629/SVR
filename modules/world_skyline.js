...
function buildLobbySprites(scene, R, wallHeight){
  ...
  for (let i = 0; i < 24; i++){
    const spr = new THREE.Sprite(new THREE.SpriteMaterial({
      ...
      transparent: true,
      opacity: 0.28 + Math.random() * 0.10,
      depthWrite: false,
      blending: THREE.AdditiveBlending
    }));
    const ang = (i / 24) * Math.PI * 2;
    const rad = R * (0.30 + Math.random() * 0.10);
    const sz = 0.28 + Math.random() * 0.70;
    spr.scale.set(sz, sz, 1);
    spr.position.set(Math.cos(ang) * rad, wallHeight * 0.18 + Math.random() * 1.8, Math.sin(ang) * rad);
  ...
  for (let i = 0; i < 96; i++){
    ...
    const rad = R * (0.18 + Math.random() * 0.42);
    spr.scale.setScalar(0.03 + Math.random() * 0.06);
    spr.position.set(Math.cos(ang) * rad, 0.30 + Math.random() * 2.0, Math.sin(ang) * rad);
    spr.userData.baseY = spr.position.y;
    spr.userData.phase = Math.random() * Math.PI * 2;
    spr.userData.rise = 0.08 + Math.random() * 0.08;
...
async function addRikiArea(scene, R, wallHeight, spawnLogoTex, log = console.log){
  ...
  const center = new THREE.Vector3(Math.cos(angle) * (R - 5.6), 0.01, Math.sin(angle) * (R - 5.6));
  const glassMat = new THREE.MeshBasicMaterial({ color: 0xd8f6ff, transparent: true, opacity: 0.08, side: THREE.DoubleSide, depthWrite: false });
  const roomW = 6.2, roomD = 4.0, roomH = 3.0;
  const front = new THREE.Mesh(new THREE.PlaneGeometry(roomW, roomH), glassMat);
  front.position.copy(center).addScaledVector(inward, 1.85).add(new THREE.Vector3(0, roomH * 0.5, 0));
  front.lookAt(center.x, roomH * 0.5, center.z);
  scene.add(front);
  for (const sgn of [-1,1]){
    const side = new THREE.Mesh(new THREE.PlaneGeometry(roomD, roomH), glassMat);
    side.position.copy(center).addScaledVector(right, sgn * (roomW * 0.5)).addScaledVector(inward, 0.0).add(new THREE.Vector3(0, roomH * 0.5, 0));
    side.lookAt(side.position.clone().addScaledVector(right, sgn));
    scene.add(side);
  }
  const roof = new THREE.Mesh(new THREE.PlaneGeometry(roomW, roomD), glassMat);
  roof.rotation.x = -Math.PI * 0.5;
  roof.position.copy(center).add(new THREE.Vector3(0, roomH, 0.0));
  scene.add(roof);

  const carpet = new THREE.Mesh(new THREE.PlaneGeometry(5.0, 2.8), new THREE.MeshStandardMaterial({ color: 0x8d1024, roughness: 0.94, metalness: 0.02, emissive: 0x30060d, emissiveIntensity: 0.20, side: THREE.DoubleSide }));
  carpet.rotation.x = -Math.PI * 0.5;
  carpet.position.copy(center).addScaledVector(inward, 0.20).add(new THREE.Vector3(0, 0.002, 0));
  scene.add(carpet);
  const walk = new THREE.Mesh(new THREE.PlaneGeometry(1.18, 2.5), new THREE.MeshStandardMaterial({ color: 0xa8132a, roughness: 0.92, metalness: 0.01, emissive: 0x450914, emissiveIntensity: 0.12, side: THREE.DoubleSide }));
  walk.rotation.x = -Math.PI * 0.5;
  walk.position.copy(center).addScaledVector(inward, 0.30).add(new THREE.Vector3(0, 0.004, 0));
  scene.add(walk);
  ...
  const sideRailX = 1.35;
  const railDepths = [1.15, 0.15, -0.85];
  [-1, 1].forEach((sgn)=>{
    railDepths.forEach((dz)=>{
      const post = new THREE.Mesh(new THREE.CylinderGeometry(0.045,0.05,0.9,14), postMat);
      post.position.copy(center).addScaledVector(right, sgn * sideRailX).addScaledVector(inward, dz).add(new THREE.Vector3(0,0.45,0));
      scene.add(post);
  ...
  [-1,1].forEach((sgn)=>{
    [[1.15,0.15],[0.15,-0.85]].forEach(([a,b])=>{
      const rope = new THREE.Mesh(new THREE.CylinderGeometry(0.024,0.024,Math.abs(a-b),12), ropeMat);
      rope.rotation.x = Math.PI * 0.5;
      rope.position.copy(center).addScaledVector(right, sgn * sideRailX).addScaledVector(inward, (a+b)*0.5).add(new THREE.Vector3(0,0.84,0));
      scene.add(rope);
    });
  });

  const signBack = new THREE.Mesh(new THREE.PlaneGeometry(4.9, 1.18), new THREE.MeshBasicMaterial({ color: 0x020203, side: THREE.DoubleSide }));
  signBack.position.copy(front.position).addScaledVector(inward, -0.02).add(new THREE.Vector3(0, 1.32, 0));
  signBack.lookAt(center.x, signBack.position.y, center.z);
  scene.add(signBack);
  const signTex = canvasTexture(1024, 256, (x,w,h)=>{
    const g = x.createLinearGradient(0,0,w,h);
    g.addColorStop(0, 'rgba(6,8,12,0.98)');
    g.addColorStop(1, 'rgba(16,18,24,0.98)');
    x.fillStyle=g; x.fillRect(0,0,w,h);
    x.strokeStyle='rgba(190,195,205,0.92)'; x.lineWidth=8; x.strokeRect(12,12,w-24,h-24);
    x.textAlign='center'; x.textBaseline='middle';
    x.fillStyle='rgba(230,235,242,0.98)'; x.font='bold 64px system-ui, Arial'; x.fillText('REIKI TIME', w/2, 88);
    x.fillStyle='rgba(255,76,90,0.98)'; x.font='bold 32px system-ui, Arial'; x.fillText('WAITING FOR AUTHORIZATION BY S.R.', w/2, 168);
  });
  const sign = new THREE.Mesh(new THREE.PlaneGeometry(4.52, 1.00), new THREE.MeshBasicMaterial({ map: signTex, transparent: true, side: THREE.DoubleSide, depthWrite: false }));
  sign.position.copy(signBack.position).addScaledVector(inward, -0.01);
  sign.lookAt(center.x, sign.position.y, center.z);
  scene.add(sign);
  if (spawnLogoTex){
    const logo = new THREE.Mesh(new THREE.PlaneGeometry(1.34, 1.34), new THREE.MeshBasicMaterial({ map: spawnLogoTex, transparent: true, side: THREE.DoubleSide, depthWrite: false }));
    logo.position.copy(sign.position).add(new THREE.Vector3(0, 0.98, 0));
  ...
  if (plantRoot){
    plantRoot.traverse((child)=>{
      ...
      if (nm.includes('pot')) child.material = new THREE.MeshStandardMaterial({ color: 0x3c1620, roughness: 0.88, metalness: 0.02, emissive: 0x17060b, emissiveIntensity: 0.06 });
...
export async function buildSkylineRoom(scene, { log = console.log } = {}){
  ...
  [
    ...
    { angle: 0, kind: "reiki", title: "REIKI TIME", subtitle: "WAITING FOR APPROVAL BY S.R.", size: [6.8, wallHeight - 0.46], logo: [1.8, 1.8], y: wallHeight * 0.5 },
  ...
  moon.position.set(-10, wallHeight + 420.0, -(R + 700));
  ...
  const moonGlow = new THREE.PointLight(0xeaf2ff, 6.0, 760, 1.5);
  scene.add(moonGlow);
  const marsGlow = new THREE.PointLight(0xff9a72, 1.0, 180, 2.0);
  scene.add(marsGlow);
  marsGlow.visible = false;
  ...
  const seats = [
    makeSeat(scene, 0, -2.48, Math.PI, "North Edge", chairMat, metalMat),
    makeSeat(scene, -2.28, -1.02, -Math.PI * 0.5, "Left Front", chairMat, metalMat),
    makeSeat(scene, -2.28, 1.02, -Math.PI * 0.5, "Left Back", chairMat, metalMat),
    makeSeat(scene, 0, 2.48, 0, "South Edge", chairMat, metalMat),
    makeSeat(scene, 2.28, -1.02, Math.PI * 0.5, "Right Front", chairMat, metalMat),
    makeSeat(scene, 2.28, 1.02, Math.PI * 0.5, "Right Back", chairMat, metalMat)
  ];
  seats.forEach((seat)=>{ seat.occupiedByBot = false; });
  ...
  if (seatedBotBase){
    ...
    botSeatIndices.forEach((seatIdx, idx)=>{
      ...
      const inward = new THREE.Vector3(-seat.x, 0, -seat.z).normalize();
      anchor.position.set(seat.x * 0.94 + inward.x * 0.06, 0.02, seat.z * 0.94 + inward.z * 0.06);
      anchor.lookAt(new THREE.Vector3(0, 1.02, 0));
      anchor.rotation.y += Math.PI;
      bot.rotation.y = Math.PI;
      anchor.add(bot);
      scene.add(anchor);
      seatedBots.push({ anchor, bot, seatIdx, phase: idx * 0.75 });
    });
  }
  ...
  scene.userData._tickWorld = (dt)=>{
    ...
    const moonOrbit = t * 0.004 + Math.PI * 0.72;
    const moonRadius = cityRadius + 380.0;
    moon.position.set(
      Math.cos(moonOrbit) * 26.0,
      wallHeight + 410.0 + Math.sin(t * 0.006) * 1.0,
      -(moonRadius * 1.02) + Math.sin(moonOrbit) * 20.0
    );
    moon.rotation.y += dt * 0.10;
    ...
    if (lobbySprites){
      ...
      lobbySprites.children.forEach((spr, i)=>{
        ...
        if (spr.material && 'opacity' in spr.material) spr.material.opacity = 0.22 + 0.10 * (0.5 + 0.5 * Math.sin(t * 1.1 + i));
      });
      if (tiny?.children){
        tiny.children.forEach((spr, i)=>{
          if (!spr || !spr.isSprite) return;
          spr.position.y += (spr.userData.rise || 0.10) * dt;
          ...
          if (spr.material && 'opacity' in spr.material) spr.material.opacity = 0.05 + 0.04 * (0.5 + 0.5 * Math.sin(t * 1.0 + i));
        });
      }
      lobbySprites.rotation.y += dt * 0.012;
    }
    ...
    seatedBots.forEach((rec)=>{
      ...
      const inward = new THREE.Vector3(-seat.x, 0, -seat.z).normalize();
      rec.anchor.position.set(seat.x * 0.94 + inward.x * 0.06, 0.02 + Math.sin(t * 1.1 + rec.phase) * 0.005, seat.z * 0.94 + inward.z * 0.06);
      rec.anchor.lookAt(new THREE.Vector3(0, 1.02, 0));
      rec.anchor.rotation.y += Math.PI;
      rec.bot.rotation.y = Math.PI;
    });

...
import { createDesktopControls } from "./modules/desktop_controls.js?v=phase168";
import { createHands } from "./modules/hands.js";
import { createTeleportRig } from "./modules/teleport.js?v=phase168";
...
const PHASE168 = "PHASE-168-LOBBY-CLEANUP-TELEPORT-LOCK";
...
let lastModeText = "";
function setMode(text){ if ($mode && text !== lastModeText){ lastModeText = text; $mode.textContent = text; } }
function log(...args){
...
window.addEventListener("unhandledrejection", (e)=>{
  ...
  if (/PointerLock|pointer lock|InvalidStateError|WrongDocumentError/i.test(text)) { console.warn("Pointer lock rejection ignored; drag-look fallback active.", e?.reason || e); return; }
...
try {
  ...
  const makeBootPlanet = (name, radius, color, glowColor, x, y, z)=>{
    ...
    group.name = `${name}_PHASE168_HIGH_SKY_BOOT_SAFE`;
  ...
  const moon = makeBootPlanet("Moon", 3.7, 0xf3f6ff, 0xdbeaff, -68, 88, -148);
  const mars = makeBootPlanet("Mars", 2.0, 0xd97954, 0xff986d, 76, 108, -172);
  scene.userData._bootPlanetTick = (dt)=>{ moon.userData.body.rotation.y += dt * 0.075; mars.userData.body.rotation.y += dt * 0.062; };
  world = { roomClamp: 18, seats: [{x:0,z:2.9,label:"Player Seat"},{x:2.3,z:1.3,label:"Bot 1"},{x:2.3,z:-1.3,label:"Bot 2"},{x:0,z:-2.9,label:"Bot 3"},{x:-2.3,z:-1.3,label:"Bot 4"},{x:-2.3,z:1.3,label:"Bot 5"}], tableCenter: new THREE.Vector3(0,0,0), joinRadius: 4.5, previewOrbitRadius: 10, sceneTargets: { lobby:{pos:{x:0,z:4.8},look:{x:0,z:0}}, seat:{pos:{x:0,z:2.9},look:{x:0,z:0}}, reiki:{pos:{x:-7,z:-4},look:{x:0,z:0}}, pga:{pos:{x:7,z:-4},look:{x:0,z:0}}, sponsor:{pos:{x:0,z:-8},look:{x:0,z:0}}, scorpion:{pos:{x:6,z:6},look:{x:0,z:0}}, legends:{pos:{x:-6,z:6},look:{x:0,z:0}} } };
}
const { roomClamp, seats, tableCenter, joinRadius, previewOrbitRadius, sceneTargets = {} } = world;
let tp = null;
const player = { getPose: ()=> tp?.getPlayerPose?.() || { x: camera.position.x, y: camera.position.y, z: camera.position.z, yaw: camera.rotation.y || 0 }, setPose: (x, y = 0, z)=> tp?.setPlayerPose?.(x, y, z), setXZ: (x, z)=> tp?.setPlayerXZ?.(x, z), setYaw: (yaw)=> tp?.setPlayerYaw?.(yaw) };
window.SVR_GAME = { scene, camera, renderer, world, player, phase: PHASE168 };
...
const audio = createAudioPlaylist({ tracks: [{ title: "Lobby 07", url: "./assets/audio/07.mp3" }], onState: (state)=>{ if (!$status || renderer.xr.isPresenting) return; if (state.error){ setStatus(`Audio: ${state.error}`); return; } if (state.enabled){ setStatus(`Now Playing: ${state.trackTitle}`); return; } setStatus(state.primed ? `Music Ready: ${state.trackTitle}` : `Audio Locked: tap once to unlock`); } });
let seated = false;
let seatIndex = -1;
let cash = 50000;
...
function joinTable(){ if (!inTableZone()) return false; const p = currentHeadXZ(); let best = 0; let bestDist = Infinity; seats.forEach((seat, idx)=>{ if (seat.label === "Dealer Side") return; const d = Math.hypot(p.x - seat.x, p.z - seat.z); if (d < bestDist){ bestDist = d; best = idx; } }); seatIndex = best; seated = true; const seat = seats[best]; if (renderer.xr.isPresenting) tp.setPlayerPose(seat.x, -0.42, seat.z); else moveDesktopToSeat(seat); setMode(`Seat: ${seat.label}`); return true; }
...
function gotoScene(key){ if (["reikiRoom","pgaDrive","pgaChipPutt","chipPutt","storeRoom","smokerLounge","scorpion"].includes(key)) return openPrivateScene(key); const rec = sceneTargets?.[key]; if (!rec?.pos) return false; movePlayerToSpot(rec.pos, rec.look || null); setStatus(`Quick jump: ${key}`, { force: true }); return true; }
$sceneButtons.forEach((btn)=>{ btn.addEventListener("click", ()=>{ const privateKey = btn.dataset.private; const key = btn.dataset.scene; if (privateKey) openPrivateScene(privateKey); else if (key) gotoScene(key); }); });
window.addEventListener("keydown", async (e)=>{ if (renderer.xr.isPresenting || e.repeat) return; if (e.code === "KeyM") await audio.toggle(); if (e.code === "KeyN") await audio.next(); if (e.code === "KeyJ") joinTable(); if (e.code === "KeyL") leaveTable(); if (e.code === "KeyT") tp.toggleMode(); if (e.code === "Digit1") gotoScene("lobby"); if (e.code === "Digit2") gotoScene("table"); if (e.code === "Digit3") gotoScene("seat"); if (e.code === "Digit4") gotoScene("reiki"); if (e.code === "Digit5") gotoScene("pga"); if (e.code === "Digit6") gotoScene("legends"); if (e.code === "Digit7") gotoScene("sponsor"); if (e.code === "Digit8") openPrivateScene("scorpion"); if (e.code === "Digit9") openPrivateScene("reikiRoom"); if (e.code === "Digit0") openPrivateScene("pgaDrive"); if (e.code === "Minus") openPrivateScene("pgaChipPutt"); });

const watch = createWristWatch({ scene, camera, renderer, getState: ()=>({ audioEnabled: audio.getState().enabled, trackTitle: audio.getState().trackTitle || "Lobby 07", cash, seated, inTableZone: inTableZone(), seatLabel: seatLabel(), teleportEnabled: tp.isEnabled ? tp.isEnabled() : true }), actions: { toggleAudio: ()=>audio.toggle(), nextTrack: ()=>audio.next(), joinTable, leaveTable, toggleTeleport: ()=>tp.toggleMode(), goLobby: ()=>gotoScene("lobby"), goTable: ()=>gotoScene("table"), goSeat: ()=>gotoScene("seat"), goReiki: ()=>gotoScene("reiki"), goPga: ()=>gotoScene("pga"), goLegend: ()=>gotoScene("legends"), goSponsor: ()=>gotoScene("sponsor"), goScorpion: ()=>openPrivateScene("scorpion"), goReikiRoom: ()=>openPrivateScene("reikiRoom"), goPgaDrive: ()=>openPrivateScene("pgaDrive"), goPgaChipPutt: ()=>openPrivateScene("pgaChipPutt") } });
...
setStatus(AUTOCAM ? "Live preview ready" : "Ready. Phase 168 active. Camera-forward teleport lock loaded. Desktop scene buttons enabled.", { force: true });
...
renderer.xr.addEventListener("sessionstart", async ()=>{ setHudVisible(false); document.getElementById("sceneNav")?.style.setProperty("display","none"); await audio.prime(); audio.stop(); await tp.onSessionStart(); });
...
const previewShots = [ { y: 1.62, r: previewOrbitRadius - 3.5, speed: 0.030, sway: 0.015, lookY: 1.02, targetX: 0.0, targetZ: 0.0, leadX: 0.14, leadZ: 0.08 }, { y: 1.68, r: previewOrbitRadius - 3.0, speed: -0.024, sway: 0.018, lookY: 1.04, targetX: 0.1, targetZ: -0.1, leadX: -0.10, leadZ: 0.12 }, { y: 1.64, r: previewOrbitRadius - 2.6, speed: 0.022, sway: 0.018, lookY: 1.03, targetX: -0.12, targetZ: 0.04, leadX: 0.10, leadZ: -0.08 } ];
renderer.setAnimationLoop(()=>{
  const now = performance.now();
  const dt = Math.min((now - tPrev) / 1000, 0.033);
  tPrev = now;
  if (!renderer.xr.isPresenting){ if (!AUTOCAM) desktop.update(dt); else { const shotIndex = Math.floor(now / 9000) % previewShots.length; const shot = previewShots[shotIndex]; const orbitT = now * 0.001 * shot.speed; previewPos.set(Math.cos(orbitT) * shot.r + shot.leadX * Math.sin(now * 0.00047), shot.y + Math.sin(now * 0.0014 + shotIndex) * shot.sway, Math.sin(orbitT) * shot.r + shot.leadZ * Math.cos(now * 0.00053)); previewTarget.set(shot.targetX, shot.lookY, shot.targetZ); camera.position.lerp(previewPos, 0.06); camera.lookAt(previewTarget); } scene.userData._camera = camera; } else scene.userData._camera = renderer.xr.getCamera(camera);

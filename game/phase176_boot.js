...
function sync(){
  ...
  window.SVR_PHASE191 = { build: LABEL, active: true, duplicateFloorAuthorityLock: true };
...
function hideSupersededVisuals(){
  ...
  scene.traverse((obj)=>{
    ...
    if (patterns.some(rx=>rx.test(name)) && !/PHASE191|PHASE190|PHASE189|PHASE185|PHASE187|PHASE181|PHASE180|PHASE178|PHASE177|PGA|REIKI|WELLNESS|SPONSOR|STORE|SCORPION|LEGEND|Watch|Teleport|Hand|Controller|Moon|Mars/i.test(name)) obj.visible = false;
  });
}

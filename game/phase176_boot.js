...
function sync(){
  ...
  window.SVR_PHASE188 = { build: LABEL, active: true, secondFloor: true };
...
function hideSupersededVisuals(){
  ...
  scene.traverse((obj)=>{
    ...
    if (patterns.some(rx=>rx.test(name)) && !/PHASE188|PHASE185|PHASE187|PHASE181|PHASE180|PHASE178|PHASE177|PGA|REIKI|WELLNESS|SPONSOR|STORE|SCORPION|LEGEND|Watch|Teleport|Hand|Controller|Moon|Mars/i.test(name)) obj.visible = false;
  });
}

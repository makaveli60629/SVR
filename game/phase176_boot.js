...
const LABEL = "UPDATE-3.0-PHASE-193-SKYLINE-POPIN-GEOMETRY-REFINE-LOCK";
...
function sync(){
  ...
  window.SVR_PHASE192 = { build: "UPDATE-3.0-PHASE-192-LOBBY-VISUAL-CLEANUP-LOCK", active: true, lobbyVisualCleanup: true };
  window.SVR_PHASE193 = { build: LABEL, active: true, skylinePopInFilter: true, refinedGeometry: true };
...
function hideSupersededVisuals(){
  ...
  scene.traverse((obj)=>{
    ...
    if (patterns.some(rx=>rx.test(name)) && !/PHASE193|PHASE192|PHASE191|PHASE190|PHASE185_OFFICIAL_POLISHED_MARBLE_FLOOR|PHASE185_FLOOR_INLAY|PHASE187|PHASE181|PHASE180|PHASE178|PHASE177|PGA|REIKI|WELLNESS|SPONSOR|STORE|SCORPION|LEGEND|Watch|Teleport|Hand|Controller|Moon|Mars/i.test(name)) obj.visible = false;
  });
}
...
setInterval(()=>{ sync(); hideSupersededVisuals(); }, 3000);
...
setTimeout(hideSupersededVisuals, 80);
setTimeout(hideSupersededVisuals, 250);

/*
 * SVR Poker Phase 250 - Boot JS Syntax Hard Reset Lock
...
(function(){
  ...
  function safeText(value){
    return String(value == null ? "" : value).replace(/[<>]/g, "").slice(0, 4000);
  }

  function softReport(title, detail){
    try {
      window.dispatchEvent(new CustomEvent("svr_boot_guard_report", {
        detail: {
          build: BUILD,
          phase: PHASE,
          title: title,
          detail: safeText(detail && (detail.stack || detail.message || detail)),
  ...
  function showRecovery(title, detail, soft){
    ...
    recEl.style.cssText = [
      ...
      "max-width:760px","margin:auto","display:block","padding:14px 16px",
      "border-radius:18px","background:rgba(5,7,18,.92)",
      "border:1px solid rgba(140,255,220,.48)","color:#eafff7",
      "box-shadow:0 18px 54px rgba(0,0,0,.68)",
      "font:13px/1.45 system-ui,Segoe UI,Arial,sans-serif"
    ].join(";");

    recEl.innerHTML =
      "<strong>SVR Boot Guard</strong><br>" +
      ...
      "<button data-svr-boot=\"fallback\" style=\"padding:8px 12px;border-radius:999px;border:1px solid #8cffdc;background:#061510;color:#eafff7;cursor:pointer\">Open Recovery Shell</button>" +
      "</div>" +
  ...
  function hideRecovery(){
    ...
    var fb = document.getElementById("svrBootFallback");
  ...
  window.SVR_BOOT_GUARD = {
    build: BUILD,
    phase: PHASE,
    startedAt: startedAt,
    ready: function(){ return ready; },
    imported: function(){ return imported; },
    recoveryShown: function(){ return recoveryShown; },
  ...
  document.addEventListener("click", function(event){
    ...
    if (action === "cache") location.href = "./index.html?v=phase250-" + Date.now();
  ...
  window.addEventListener("error", function(event){
    if (!ready) {
      var detail = event.error || event.message || event;
      softReport("Runtime error before ready signal.", detail);
      showRecovery("Runtime error before ready signal.", detail, false);
      startFallback("Runtime error before ready signal.", detail);
    }
  });

  window.addEventListener("unhandledrejection", function(event){
    if (!ready) {
      var detail = event.reason || event;
      softReport("Promise rejection before ready signal.", detail);
      showRecovery("Promise rejection before ready signal.", detail, false);
      startFallback("Promise rejection before ready signal.", detail);
    }
  });
  ...
  setTimeout(function(){
    if (!ready) {
      showRecovery("Game is taking longer than expected. Recovery shell and route recovery are available.", "", true);
      if (window.SVR_BOOT_ROUTE_RECOVERY) window.SVR_BOOT_ROUTE_RECOVERY.collect("slow-boot");
    }
  }, 6000);

  mainTimer = setTimeout(function(){
    if (!ready) {
      showRecovery("Game did not send ready signal. Opening recovery shell so Booting cannot freeze.", "", false);
      startFallback("Main runtime timeout before ready signal.", "Likely CDN/import/cache/runtime module issue.");
    }
  }, 15000);

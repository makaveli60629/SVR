 ...
 */
(function(){
  var BUILD = 'PHASE-232-BRIDGE-ALIAS-RECOVERY-LOCK';
  ...
  function startFallback(reason, detail){
    ...
    import('./modules/boot_fallback.js?v=phase232').then(function(){
  ...
  import('./modules/boot_route_recovery.js?v=phase232').catch(function(error){ softReport('Boot route recovery module failed.', error); });
  import('./modules/boot_route_health.js?v=phase232').catch(function(error){ softReport('Boot route health module failed.', error); });
  ...
  document.addEventListener('click', function(event){
    ...
    if (action === 'cache') location.href = './index.html?v=phase232-' + Date.now();
  ...
  import('./main.js?v=phase232').then(function(){ imported = true; setStatus('Game module loaded. Building scene…'); }).catch(function(error){ imported = false; showRecovery('Failed to import main game module. Booting screen stopped by recovery guard.', error, false); startFallback('Failed to import main game module.', error); });
})();

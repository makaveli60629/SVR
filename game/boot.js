/*
 * SVR Poker Phase 202 - Boot Fallback CDN Recovery Lock
 * Purpose: never leave the player stuck on Booting; provide CDN/import fallback report.
 */
(function(){
  var BUILD = 'PHASE-204-EVENT-FIREWALL-BRIDGE-HARDENING-LOCK';
  ...
  function startFallback(reason, detail){
    ...
    import('./modules/boot_fallback.js?v=phase204').then(function(){
  ...
  document.addEventListener('click', function(event){
    ...
    if (action === 'cache') location.href = './index.html?v=phase204-' + Date.now();
  ...
  import('./main.js?v=phase204').then(function(){ imported = true; setStatus('Game module loaded. Building scene…'); }).catch(function(error){ imported = false; showRecovery('Failed to import main game module. Booting screen stopped by recovery guard.', error, false); startFallback('Failed to import main game module.', error); });
})();

...
export function applyReikiHologramPhase133({scene,camera,renderer,sceneTargets,setStatus=()=>{},log=()=>{}}={}){
  ...
  const wallCenter=rec.look.clone();wallCenter.y=0;const group=new THREE.Group();group.name="PHASE157 REIKI STORE PLACEHOLDER NO VIDEO";group.position.copy(wallCenter);
  ...
  const right=new THREE.Mesh(new THREE.PlaneGeometry(2.28,3.42),new THREE.MeshBasicMaterial({map:makeTexture(900,1200,"PROFILE SLOT",["Reserved sponsor area","Approved sponsor only","Placeholder active"],"#b58cff"),transparent:true,side:THREE.DoubleSide,depthWrite:false}));right.position.set(3.78,2.78,-.18);group.add(right);
  const slides=[makeTexture(900,1200,"STORE OPEN",["Reiki store remains","Video removed","Sponsor slot blank","Registry controlled"],"#7dfff0"),makeTexture(900,1200,"ABOUT",["Future sponsor slot","Approved copy only","No active media"]),makeTexture(900,1200,"SPONSOR",["Add through registry","Remove instantly","Owner approval required"],"#7dffb2")];
  let index=0;const displayMat=new THREE.MeshBasicMaterial({map:slides[0],transparent:true,opacity:.92,side:THREE.DoubleSide,depthWrite:false});const display=new THREE.Mesh(new THREE.PlaneGeometry(2.20,3.50),displayMat);display.position.set(0,2.92,-.62);group.add(display);const frame=new THREE.Mesh(new THREE.BoxGeometry(2.42,3.75,.08),teal);frame.position.set(0,2.92,-.66);group.add(frame);
  ...
  function setSlide(n){index=(n+slides.length)%slides.length;displayMat.map=slides[index];displayMat.needsUpdate=true;setStatus(`Reiki store slide ${index+1}/${slides.length}`,{force:true});}
  ...
  const oldTick=scene.userData._tickWorld;scene.userData._tickWorld=dt=>{oldTick?.(dt);display.position.y=2.92+Math.sin(performance.now()*.002)*.03;};
  scene.userData._phase135ReikiWallAligned={group,setSlide};window.SVR_PHASE157_REIKI_NO_VIDEO=true;log?.("Phase 157 Reiki store placeholder no video active");setStatus("Phase 157 Reiki store placeholder no video active",{force:true});return scene.userData._phase135ReikiWallAligned;
}

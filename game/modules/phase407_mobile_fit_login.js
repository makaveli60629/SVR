/* PHASE-407-MOBILE-FIT-LOGIN-BURN-LOCK | Phase 408 inline burn compatible | Phase 409 turn/fit layer | Phase 410 human input layer | Phase 411 tournament/bot/action layer | Phase 412 account gate/pro polish | Phase 420 no-downgrade navigation */
const BUILD='PHASE-407-MOBILE-FIT-LOGIN-BURN-LOCK';
const state={build:BUILD,installed:false,burnBesideBoard:false,oneBurn:false,userTurn:false,loginButton:false,accountMode:'loading',signedIn:false,thinking:false,lastThinkDelay:0,phase409Layer:false,phase410Layer:false,phase411Layer:false,phase412Layer:false,lastError:null};
...
function ensureAccountUi(){
  ...
  const snap=accountSnapshot(),profile=snap?.profile||null,apiConfigured=Boolean(snap?.apiConfigured||snap?.config?.apiBase),returnTo=encodeURIComponent(`${location.pathname}${location.search}`);
  state.accountMode=snap?.mode||'loading';state.signedIn=Boolean(profile);
  if(profile){button.textContent=`ACCOUNT • ${(profile.displayName||profile.name||'PLAYER').toUpperCase()}`;button.href='/site/profile.html?v=phase412&source=mobile-game';button.title='Player profile'}
  else{button.textContent=apiConfigured?'SIGN IN':'SIGN IN • LOCAL TEST';button.href=`/site/login.html?next=/game/android.html?channel=stable%26v=phase412&return=${returnTo}&v=phase412`;button.title=apiConfigured?'Sign in to your SVR Poker account':'Create a local test profile while the secure account backend is being configured.'}
...
function addStyle(href,key,phase='phase411'){let link=document.querySelector(`link[href*="${key}"]`);if(!link){link=document.createElement('link');link.rel='stylesheet';link.href=href;link.dataset[phase]='1';document.head.appendChild(link)}return link}
function addModule(src,key,qa,phase='phase411'){let script=document.querySelector(`script[src*="${key}"]`);if(!window[qa]&&!script){script=document.createElement('script');script.type='module';script.src=src;script.dataset[phase]='1';document.body.appendChild(script)}return script||window[qa]}
function phaseNumber(value){const match=String(value||'').match(/phase(?:-|_)?(\d+)/i);return match?Number(match[1]):0}
function ensureMinimumPhaseVersion(anchor,minPhase){try{const u=new URL(anchor.href,location.href),current=phaseNumber(u.searchParams.get('v'));if(current<minPhase){u.searchParams.set('v',`phase${minPhase}`);anchor.href=u.pathname+u.search+u.hash;return true}}catch{}return false}
...
function ensurePhase412Layer(){
  const polishStyle=addStyle('/game/styles/phase412_professional_polish.css?v=phase412','phase412_professional_polish.css','phase412');
  const gate=addModule('/game/modules/phase412_tournament_account_gate.js?v=phase412','phase412_tournament_account_gate.js','SVR_PHASE412_TOURNAMENT_ACCOUNT_QA','phase412');
  const polish=addModule('/game/modules/phase412_professional_polish.js?v=phase412','phase412_professional_polish.js','SVR_PHASE412_PROFESSIONAL_QA','phase412');
  document.querySelectorAll('a[href*="tournaments.html"]').forEach(a=>ensureMinimumPhaseVersion(a,412));
  state.phase412Layer=Boolean(polishStyle&&gate&&polish);return state.phase412Layer
}
function poll(){try{ensurePhase409Layer();ensurePhase410Layer();ensurePhase411Layer();ensurePhase412Layer();ensureSingleBurn();ensureAccountUi();turnPolish();thinkingPolish();micPolish();state.installed=Boolean(state.oneBurn&&state.burnBesideBoard&&state.loginButton&&state.phase409Layer&&state.phase410Layer&&state.phase411Layer&&state.phase412Layer);state.lastError=null}catch(e){state.lastError=String(e?.message||e)}}

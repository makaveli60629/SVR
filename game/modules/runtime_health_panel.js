...
// Game-side only. Syncs the health panel with the latest Phase 98 runtime audit.
// No website/site edits.
...
function injectStyle(){
  ...
  style.textContent = `
    ...
    #svrHealthPanel { position: fixed; left: 12px; bottom: 108px; z-index: 45; width: min(430px, calc(100vw - 24px)); max-height: min(62vh, 520px); overflow: auto; border: 1px solid rgba(180,140,255,.46); border-radius: 18px; background: linear-gradient(135deg, rgba(4,8,16,.91), rgba(24,9,42,.92)); color: #f5f2ff; box-shadow: 0 18px 50px rgba(0,0,0,.48); backdrop-filter: blur(10px); padding: 12px; font-family: system-ui, -apple-system, Segoe UI, Roboto, Arial, sans-serif; pointer-events: auto; display: none; contain: layout paint style; }
...
function audit(){ return window.SVR_PHASE98_RUNTIME_AUDIT || window.SVR_PHASE96_RUNTIME_AUDIT || window.SVR_PHASE94_RUNTIME_AUDIT || window.SVR_PHASE93_RUNTIME_AUDIT || null; }
...
function render(){
  ...
  body.innerHTML = `
    ...
    <div class="line"><span class="key">HUD / FX</span><span class="val">HUD ${yesNo(modules.pokerHud)} • Table ${yesNo(modules.tableFx)} • NPC ${yesNo(modules.npcFx)} • Feedback ${yesNo(modules.feedbackFx)} • Panel ${yesNo(modules.healthPanel)}</span></div>
...
function boot(){
  if (window.SVR_PHASE98_RUNTIME_HEALTH_PANEL) return;
  makePanel();
  window.SVR_PHASE98_RUNTIME_HEALTH_PANEL = { phase: PHASE, render, open: () => root?.classList.add("svr-open"), close: () => root?.classList.remove("svr-open") };
  window.SVR_PHASE97_RUNTIME_HEALTH_PANEL = window.SVR_PHASE98_RUNTIME_HEALTH_PANEL;

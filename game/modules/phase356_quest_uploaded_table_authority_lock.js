...
export const BUILD = 'PHASE-356-QUEST-UPLOADED-TABLE-AUTHORITY-LOCK';
...
function safeFind(root, name) {
  if (typeof window.SVR_PHASE356_SAFE_FIND === 'function') return window.SVR_PHASE356_SAFE_FIND(root, name);
  let found = null;
...
function removeCompetingTables(host) {
  ...
  safeWalk(host, (object) => {
    ...
    if (/^(?:PHASE356_QUEST_TABLE_FALLBACK|PHASE159_FBX_TABLE_FLAT_SCALE_FIX_ROOT|PHASE158_ACTUAL_FBX_TABLE_SCALE_ROOT|PHASE157_ACTUAL_FBX_TABLE_ROOT|PHASE200_INTENDED_LOBBY_POKER_TABLE_LOCKED)$/.test(object.name)) {
      remove.push(object);
    }
  });
  for (const object of remove) object.removeFromParent?.();
  state.fallbackRemoved = remove.some((object) => object.name === 'PHASE356_QUEST_TABLE_FALLBACK');
}
...
async function install() {
  ...
  try {
    ...
    if (!scene) throw new Error('QUEST_SCENE_NOT_READY');
    ...
    window.SVR_PHASE356_UPLOADED_TABLE_ROOT = authorityRoot;
    state.loaded = true;
    ...
    window.SVR_PHASE356_UPLOADED_TABLE_STATE = { ...state };
  }
}

window.SVR_PHASE356_UPLOADED_TABLE_QA = () => {

...
(function(){
  ...
  async function verify(){
    ...
    try {
      const res = await fetch('./version.json?v=phase244-' + Date.now(), { cache: 'no-store' });
    ...
    try {
      ...
      mark('boot-cache-query', scripts.includes('boot.js?v=phase244'), scripts || 'No script src found.');
    } catch (err) {

...
(function(){
  ...
  async function verify(){
    ...
    try {
      const res = await fetch('./version.json?v=phase242-' + Date.now(), { cache: 'no-store' });
    ...
    try {
      ...
      mark('boot-cache-query', scripts.includes('boot.js?v=phase242'), scripts || 'No script src found.');
    } catch (err) {

/*
 * SVR Poker Phase 210 - Boot Cache Marker Alignment Watchdog
...
(function(){
  ...
  async function verify(){
    ...
    mark('hud-build-marker', hudText.includes(BUILD), 'HUD should show the Phase 210 build marker.');
    try {
      const res = await fetch('./version.json?v=phase210-' + Date.now(), { cache: 'no-store' });
    ...
    try {
      ...
      mark('boot-cache-query', scripts.includes('boot.js?v=phase210'), scripts || 'No script src found.');
    } catch (err) {

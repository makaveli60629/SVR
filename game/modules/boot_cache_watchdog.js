...
(function(){
  ...
  async function verify(){
    ...
    try {
      const res = await fetch('./version.json?v=phase234-' + Date.now(), { cache: 'no-store' });
    ...
    try {
      ...
      mark('boot-cache-query', scripts.includes('boot.js?v=phase234'), scripts || 'No script src found.');
    } catch (err) {

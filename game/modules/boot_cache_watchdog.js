/*
 * SVR Poker Phase 212 - Boot Cache Marker Alignment Watchdog
...
(function(){
  ...
  async function verify(){
    ...
    mark('hud-build-marker', hudText.includes(BUILD), 'HUD should show the Phase 212 build marker.');
    try {
      const res = await fetch('./version.json?v=phase212-' + Date.now(), { cache: 'no-store' });
    ...
    try {
      ...
      mark('boot-cache-query', scripts.includes('boot.js?v=phase212'), scripts || 'No script src found.');
    } catch (err) {

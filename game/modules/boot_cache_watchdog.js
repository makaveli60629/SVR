/*
 * SVR Poker Phase 211 - Boot Cache Marker Alignment Watchdog
...
(function(){
  ...
  async function verify(){
    ...
    mark('hud-build-marker', hudText.includes(BUILD), 'HUD should show the Phase 211 build marker.');
    try {
      const res = await fetch('./version.json?v=phase211-' + Date.now(), { cache: 'no-store' });
    ...
    try {
      ...
      mark('boot-cache-query', scripts.includes('boot.js?v=phase211'), scripts || 'No script src found.');
    } catch (err) {

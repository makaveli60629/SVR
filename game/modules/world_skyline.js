...
async function addRikiArea(scene, R, wallHeight, spawnLogoTex, log = console.log){
  ...
  const founderPanelTex = canvasTexture(900, 1200, (x,w,h)=>{
    ...
    let y = 258;
    y = fillWrappedText(x, 'Partner branding is awaiting approval. This Reiki hub remains neutral, modular, and sponsor-ready.', 60, y, w - 120, 42);
    y += 78;
    ...
    ['Reiki-style meditation sessions', 'Meditation and reset support', 'Wellness partner referrals', 'Holistic wellness guidance'].forEach((line)=>{ x.fillText('• ' + line, 72, y); y += 46; });
    y += 40;
    x.fillStyle = '#bcffe9'; x.font = 'bold 40px system-ui, Arial'; x.fillText('Book / Explore', 60, y);
    y += 54;
    x.fillStyle = '#f6ffff'; x.font = '34px system-ui, Arial';
    fillWrappedText(x, 'approval pending • sponsor-ready • Reiki VR entry', 60, y, w - 120, 40);
  });
  ...
  const rightInfoTex = canvasTexture(900, 520, (x,w,h)=>{
    ...
    let y = 148;
    ['Awaiting approval', 'Reiki and wellness focus', 'Meditation-forward client care', 'Virtual hub ready for growth'].forEach((line)=>{ x.fillText('• ' + line, 46, y); y += 70; });
  });
  ...
  const centerInfoTex = canvasTexture(900, 1200, (x,w,h)=>{
    ...
    x.fillStyle = '#bcffe9'; x.font = 'bold 34px system-ui, Arial'; x.fillText('Neutral Reiki / Wellness Hub', w/2, 166);
    x.fillStyle = '#f6ffff'; x.font = 'bold 46px system-ui, Arial'; x.fillText('Wellness Services', w/2, 296);
    ...
    x.fillStyle = '#bcffe9'; x.font = '60px system-ui, Arial'; x.fillText('Wellness • Reiki • Meditation', w/2, 788);
    x.fillStyle = '#d4fff3'; x.font = '31px system-ui, Arial'; x.fillText('Mindful reset • future approved partner storefront', w/2, 856);
  });
...
function buildStoreWall(scene, R, wallHeight, spawnLogoTex){
  ...
  const adHeader = new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({ map: createSponsorPlateTexture('AWAITING APPROVAL', 'sponsor placeholder'), transparent: true, side: THREE.DoubleSide })
  );

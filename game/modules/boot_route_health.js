...
(function(){
  ...
  const ROUTES = [
    { key:'lobby', label:'Lobby', url:'./index.html?v=phase227-routecheck' },
    { key:'scorpion', label:'Scorpion', url:'./scorpion.html?v=phase227-routecheck' },
    { key:'pgaDrive', label:'PGA Drive', url:'./pga-drive.html?v=phase227-routecheck' },
    { key:'chipPutt', label:'Chip/Putt', url:'./chip-putt.html?v=phase227-routecheck' },
    { key:'reiki', label:'Reiki Room', url:'./reiki.html?v=phase227-routecheck' },
    { key:'store', label:'VR Store', url:'./store-room.html?v=phase227-routecheck' },
    { key:'smoker', label:'Smoker Lounge', url:'./smoker-lounge.html?v=phase227-routecheck' },
    { key:'cam3', label:'Cam 3 Preview', url:'./cam3.html?v=phase227-routecheck' }
  ];
  let lastResult = null;
  let overlay = null;
  ...
  function download(){ const data=JSON.stringify(lastResult || {build:BUILD, at:now(), results:[]}, null, 2); const a=document.createElement('a'); a.href=URL.createObjectURL(new Blob([data],{type:'application/json'})); a.download='svr-route-health-phase227.json'; a.click(); setTimeout(()=>URL.revokeObjectURL(a.href), 1000); }

...
export function createTeleportRig({ scene, renderer, camera, roomClamp, log=console.log }){
  ...
  function maintainStanding(){
    ...
    window.SVR_PHASE101J_STANDING_LOCK={build:LABEL,standing:!seated,headY:Number(vHead.y.toFixed(2)),targetY:Number(desired.toFixed(2)),playerY:Number(playerY.toFixed(2)),checkedAt:new Date().toISOString()};
  }
  ...
  function snapTurnInPlace(delta){
    ...
    window.SVR_PHASE101J_SNAP_TURN={build:LABEL,yaw:Number(playerYaw.toFixed(3)),inPlace:true,checkedAt:new Date().toISOString()};
  }
  ...
  function movementForward(){const f=headForward().clone(); if(f.lengthSq()<1e-5) return yawForward().clone(); window.SVR_PHASE101J_MOVE_FORWARD_SOURCE={build:LABEL,source:"xr-head-camera",x:Number(f.x.toFixed(3)),z:Number(f.z.toFixed(3)),checkedAt:new Date().toISOString()}; return f;}
  ...
  function forwardLockedDirection(candidate, source){
    ...
    window.SVR_PHASE101J_AIM_FORWARD_LOCK={build:LABEL,source,beforeDot,afterDot,corrected,x:Number(d.x.toFixed(3)),z:Number(d.z.toFixed(3)),headX:Number(hf.x.toFixed(3)),headZ:Number(hf.z.toFixed(3)),checkedAt:new Date().toISOString()};
    return d;
  }

  function magneticTarget(t){let best=null,bestD=2.85; scene.traverse(o=>{ if(!o?.userData?.svrMagnetTarget && !/^UPDATE31[DFG]_MAGNET_/i.test(String(o.name||""))) return; const p=new THREE.Vector3(); o.getWorldPosition(p); const d=Math.hypot(p.x-t.x,p.z-t.z); if(d<bestD){bestD=d; best=p;} }); if(!best){ window.SVR_PHASE101J_MAGNETIC_LOCK=null; return null; } const target=clampTarget(best.x,best.z).clone(); window.SVR_PHASE101J_MAGNETIC_LOCK={build:LABEL,active:true,x:Number(target.x.toFixed(2)),z:Number(target.z.toFixed(2)),dist:Number(bestD.toFixed(2)),checkedAt:new Date().toISOString()}; return target;}
  function aimAxisDistance(gR,gL,r,l,dt){const candidates=[r.y,l.y,r.x,l.x].filter(v=>Math.abs(v)>.14); if(!candidates.length) return; const v=candidates.sort((a,b)=>Math.abs(b)-Math.abs(a))[0]; aimDist=THREE.MathUtils.clamp(aimDist+(-v)*13.5*dt,1.2,26.0); window.SVR_PHASE101J_AIM_DISTANCE=Number(aimDist.toFixed(2));}
  function moveControllers(dt){const gR=gp(rightControllerRef),gL=gp(leftControllerRef),r=stick(gR,"right"),l=stick(gL,"left"),now=performance.now(); if(mode){aimAxisDistance(gR,gL,r,l,dt);return;} if(Math.abs(r.x)>.76&&now>snapCooldownUntil){snapTurnInPlace(Math.sign(r.x)*Math.PI/4);snapCooldownUntil=now+320;} const y=Math.abs(r.y)>.14?r.y:l.y;if(Math.abs(y)<.14)return; const f=movementForward().clone(),move=-y,speed=4.15; setPlayerXZ(playerX+f.x*move*speed*dt,playerZ+f.z*move*speed*dt); window.SVR_PHASE101J_MOVE_VECTOR={build:LABEL,x:Number(f.x.toFixed(3)),z:Number(f.z.toFixed(3)),yaw:Number(playerYaw.toFixed(3)),checkedAt:new Date().toISOString()};}

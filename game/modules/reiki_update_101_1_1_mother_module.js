...
function makeTexture(title, lines = [], opts = {}) {
  ...
  lines.forEach((line, i) => {
    x.fillText(line, c.width / 2, (opts.startY || 172) + i * (opts.gap || 42), c.width - 96);
  });
...
function purgeOldReiki(scene) {
  ...
  scene.traverse((o) => {
    ...
    if (/REIKI|RIKI|RICI|TRUEITIVE|TRUITIVE|SHYONA|ROYSTON|CHAKRA|FOUNDER/i.test(n)) {
      if (!/SVR_RICI_UPDATE_101/i.test(n)) remove.push(o);
    }
  });
  ...
  document.querySelectorAll("video").forEach((v) => {
    ...
    if (src.includes("reiki") || src.includes("hologram")) {
...
function addPlant(root, x, z, label) {
  ...
  cyl(g, `SVR_RICI_UPDATE_101_PLANT_POT_${label}`, .17, .34, [0, .17, 0], basicMat(0x2a1b1b, { roughness: .78 }), 24);

  const leafMat = basicMat(0x1f7a40, { roughness: .70, emissive: 0x062714, emissiveIntensity: .13 });
  for (let i = 0; i < 10; i++) {
...
function buildStorefront(root, packet) {
  ...
  const carpet = basicMat(0x960018, { roughness: .82, metalness: .02, emissive: 0x2d0008, emissiveIntensity: .22 });
  carpet.side = THREE.DoubleSide;
  ...
  for (let i = 0; i < zs.length - 1; i++) {
    addRope(root, left[i], left[i + 1], `L${i}`);
    addRope(root, right[i], right[i + 1], `R${i}`);
  }
  ...
  plane(root, "SVR_RICI_UPDATE_101_MAIN_SIGN", [5.70, .98], [0, 6.08, F + .08], panelMat(
    makeTexture(title, [domain, approval], {
      w: 1300,
      h: 330,
      titleFont: "900 54px system-ui,Arial",
      lineFont: "900 28px system-ui,Arial",
      titleY: 88,
      startY: 178,
      gap: 44,
      border: packet.approval?.approved ? "rgba(0,255,204,.85)" : "rgba(255,40,55,.95)",
      lineColor: packet.approval?.approved ? "#cffff8" : "#ff4050"
    })
  ), [0,0,0], 245);

  plane(root, "SVR_RICI_UPDATE_101_LEFT_INFO_WALL", [3.25, 3.8], [-4.95, 2.55, B + .04], panelMat(
    makeTexture("PRESENTATION MOCKUP", [
      "Client content is modular",
      "Branding can be removed",
      "Packet-driven sponsor system",
      "No official approval yet"
    ], {
      w: 1000,
      h: 980,
      titleFont: "900 48px system-ui,Arial",
      lineFont: "760 30px system-ui,Arial",
      titleY: 135,
      startY: 255,
      gap: 86,
      border: "rgba(255,40,55,.90)",
      lineColor: "#ffd8dc"
    })
  ), [0,0,0], 235);

  plane(root, "SVR_RICI_UPDATE_101_RIGHT_PROFILE_WALL", [3.25, 3.8], [4.95, 2.55, B + .04], panelMat(
    makeTexture("MODULAR HUB", [
      "Hologram carousel",
      "Portal destination",
      "VIP promo ready",
      "Nationwide packet ready"
    ], {
      w: 1000,
      h: 980,
      titleFont: "900 48px system-ui,Arial",
...
function cardTexture(card, packet) {
  ...
  return makeTexture(card.title || "INFO", [...(card.lines || []), approval], {
    w: 900,
    h: 1160,
    titleFont: "900 58px system-ui,Arial",
    lineFont: "760 30px system-ui,Arial",
    titleY: 140,
    startY: 265,
    gap: 72,
    border: card.type === "portal" ? "rgba(0,204,255,.90)" : card.type === "promo" ? "rgba(255,212,80,.90)" : "rgba(0,255,204,.82)"
  });
}
...
function buildActivationRoom(root, scene, camera, renderer, gotoScene, packet) {
  ...
  plane(room, "SVR_RICI_UPDATE_101_SWIPE_HELP", [2.4, .48], [0, 3.55, .15], panelMat(
    makeTexture("SWIPE  ⇠  ⇢", ["Q / E or select arrows"], {
      w: 900,
      h: 220,
      titleFont: "900 42px system-ui,Arial",
      lineFont: "760 24px system-ui,Arial",
      titleY: 70,
      startY: 145,
  ...
  const prev = plane(room, "SVR_RICI_UPDATE_101_PREV_BUTTON", [.55, .55], [-1.25, 1.60, .22], panelMat(
    makeTexture("⇠", ["PREV"], { w: 300, h: 300, titleFont: "900 96px system-ui,Arial", lineFont: "900 30px system-ui,Arial", titleY: 105, startY: 212 }), .95
  ), [0,0,0], 285);

  const next = plane(room, "SVR_RICI_UPDATE_101_NEXT_BUTTON", [.55, .55], [1.25, 1.60, .22], panelMat(
    makeTexture("⇢", ["NEXT"], { w: 300, h: 300, titleFont: "900 96px system-ui,Arial", lineFont: "900 30px system-ui,Arial", titleY: 105, startY: 212 }), .95
  ), [0,0,0], 285);

  const activate = plane(room, "SVR_RICI_UPDATE_101_ACTIVATE_BUTTON", [1.35, .42], [0, .78, .26], panelMat(
    makeTexture("ACTIVATE", ["selected card"], { w: 700, h: 240, titleFont: "900 46px system-ui,Arial", lineFont: "800 26px system-ui,Arial", titleY: 82, startY: 155, border: "rgba(0,204,255,.86)" }), .96
  ), [0,0,0], 286);

  function setCard(i) {
    active = (i + cards.length) % cards.length;
    cardMesh.material.map = cardTexture(cards[active], packet);
  ...
  function activateCard() {
    ...
    if (card.type === "portal") {
      if (gotoScene?.("reikiRoom")) return;
      window.location.href = packet.portal?.href || "./reiki.html?v=rici-101-1-1";
    }
    if (card.type === "store") {
      window.open(packet.store?.url || "https://svrpoker.com/site/store.html", "_blank", "noopener,noreferrer");
    }
  }
  ...
  window.addEventListener("keydown", (e) => {
    if (e.code === "KeyQ") setCard(active - 1);
    if (e.code === "KeyE") setCard(active + 1);
    if (e.code === "Enter") activateCard();
  });
  ...
  room.userData.tick = (t) => {
    ring.rotation.z = t * .00020;
    cardGlow.material.opacity = .10 + Math.sin(t * .002) * .035;
    cardMesh.position.y = 1.72 + Math.sin(t * .0018) * .025;
  };
...
async function loadPacket() {
  try {
    ...
    return {
      build: BUILD,
      approval: { approved: false, statusText: "AWAITING APPROVAL" },
      brand: { displayName: "RICI / REIKI HUB", domain: "PARTNER DOMAIN PENDING" },
      store: { url: "https://svrpoker.com/site/store.html" },
      portal: { href: "./reiki.html?v=rici-101-1-1" }
    };
  }
}

export async function applyRiciUpdate101MotherModule(scene, {
  log = console.log,
  gotoScene = null,
  camera = null,
  renderer = null
  ...
  scene.onBeforeRender = function(...args) {
    ...
    activation.userData.tick?.(performance.now());
  };

  window.SVR_RICI_UPDATE_101_1_1 = {
    build: BUILD,
    packet,
    removedOldReikiObjects: removed,
    approvalSafe: !packet.approval?.approved,
    swappablePacket: true,

(function(){
  ...
  const state = {
    ...
    checklist: [
      ...
      { label: "version.json shows Phase 233", done: false },
  ...
  async function run(){
    ...
    const phaseVisible = combined.includes("PHASE-222") || combined.includes('"phase":222') || combined.includes('"phase": 233');
    ...
    state.checklist = state.checklist.map(item => {
      ...
      if(item.label.includes("Cache-bust")) return {...item, done: location.search.includes("phase233")};

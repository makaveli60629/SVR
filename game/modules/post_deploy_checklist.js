(function(){
  ...
  const state = {
    ...
    checklist: [
      ...
      { label: "version.json shows Phase 226", done: false },
  ...
  async function get(url){
    const full = url + (url.includes("?") ? "&" : "?") + "v=phase226-" + Date.now();
    try {
  ...
  async function run(){
    ...
    const phaseVisible = combined.includes("PHASE-222") || combined.includes('"phase":222') || combined.includes('"phase": 226');
    ...
    state.checklist = state.checklist.map(item => {
      ...
      if(item.label.includes("Cache-bust")) return {...item, done: location.search.includes("phase226")};

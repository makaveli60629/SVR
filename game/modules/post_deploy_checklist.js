(function(){
  ...
  const state = {
    ...
    checklist: [
      ...
      { label: "version.json shows Phase 225", done: false },
  ...
  async function get(url){
    const full = url + (url.includes("?") ? "&" : "?") + "v=phase225-" + Date.now();
    try {
  ...
  async function run(){
    ...
    const phaseVisible = combined.includes("PHASE-222") || combined.includes('"phase":222') || combined.includes('"phase": 225');
    ...
    state.checklist = state.checklist.map(item => {
      ...
      if(item.label.includes("Cache-bust")) return {...item, done: location.search.includes("phase225")};

...
export function createTeleportRig({ scene, renderer, camera, roomClamp, log=console.log }){
  ...
  function floorHeightAt(x,z){
    const f=window.SVR_PHASE226_FLOOR_HEIGHT||window.SVR_PHASE224_FLOOR_HEIGHT||window.SVR_PHASE215_FLOOR_HEIGHT||window.SVR_PHASE214_FLOOR_HEIGHT||window.SVR_PHASE213_FLOOR_HEIGHT||window.SVR_PHASE212_FLOOR_HEIGHT;
  ...
  function applyReferenceSpace(){
    ...
    try{const h=-playerYaw*.5; const xf=new XRRigidTransform({x:-playerX,y:-playerY,z:-playerZ},{x:0,y:Math.sin(h),z:0,w:Math.cos(h)}); renderer.xr.setReferenceSpace(baseRefSpace.getOffsetReferenceSpace(xf)); return true;}catch(e){log("[3.1-F] ref failed",e?.message||e); return false;}
  }
  ...
  function maintainStanding(){
    ...
    window.SVR_PHASE226_STANDING_LOCK={build:LABEL,standing:!seated,headY:Number(vHead.y.toFixed(2)),targetY:Number(desired.toFixed(2)),playerY:Number(playerY.toFixed(2)),checkedAt:new Date().toISOString()};
  }
  ...
  function movementForward(){
    ...
    if(f.lengthSq()<1e-5) return yawForward().clone();
    window.SVR_PHASE226_MOVE_FORWARD_SOURCE = { build:LABEL, source:"xr-head-camera", x:Number(f.x.toFixed(3)), z:Number(f.z.toFixed(3)), checkedAt:new Date().toISOString() };
    return f;
  }
  function magneticTarget(t){
    let best=null,bestD=2.85;
    scene.traverse(o=>{ if(!o?.userData?.svrMagnetTarget && !/^UPDATE31[DF]_MAGNET_/i.test(String(o.name||""))) return; const p=new THREE.Vector3(); o.getWorldPosition(p); const d=Math.hypot(p.x-t.x,p.z-t.z); if(d<bestD){bestD=d; best=p;} });
    if(!best){ window.SVR_PHASE226_MAGNETIC_LOCK=null; return null; }
    const target=clampTarget(best.x,best.z).clone();
    window.SVR_PHASE226_MAGNETIC_LOCK={build:LABEL,active:true,x:Number(target.x.toFixed(2)),z:Number(target.z.toFixed(2)),dist:Number(bestD.toFixed(2)),checkedAt:new Date().toISOString()};
    return target;
  }
  function aimAxisDistance(gR,gL,r,l,dt){
    const candidates=[r.y,l.y,r.x,l.x].filter(v=>Math.abs(v)>.14);
    if(!candidates.length) return;
    const v=candidates.sort((a,b)=>Math.abs(b)-Math.abs(a))[0];
    aimDist=THREE.MathUtils.clamp(aimDist + (-v)*13.5*dt,1.2,26.0);
    window.SVR_PHASE226_AIM_DISTANCE=Number(aimDist.toFixed(2));
  }
  function moveControllers(dt){
    ...
    if(mode){ aimAxisDistance(gR,gL,r,l,dt); return; }
    ...
    window.SVR_PHASE226_MOVE_VECTOR={build:LABEL,x:Number(f.x.toFixed(3)),z:Number(f.z.toFixed(3)),yaw:Number(playerYaw.toFixed(3)),checkedAt:new Date().toISOString()};
  }
  ...
  function teleportByDelta(target){if(!renderer?.xr?.isPresenting||!baseRefSpace)return false;try{const xr=renderer.xr.getCamera(camera);if(!xr)return false;xr.getWorldPosition(vHead);const p=solidXZ(playerX+(target.x-vHead.x),playerZ+(target.z-vHead.z));playerX=p.x;playerZ=p.z;if(!seated)playerY=floorHeightAt(playerX,playerZ);return applyReferenceSpace();}catch(e){log("[3.1-F] teleport failed",e?.message||e);return false;}}

...
export function createTeleportRig({ scene, renderer, camera, roomClamp, log=console.log }){
  ...
  const vHead=new THREE.Vector3(), vHead2=new THREE.Vector3(), vForward=new THREE.Vector3(), vOrigin=new THREE.Vector3(), vDir=new THREE.Vector3(), vTarget=new THREE.Vector3(), vSmooth=new THREE.Vector3(CONFIG.SPAWN_X,0,CONFIG.SPAWN_Z), vWrist=new THREE.Vector3(), vIndex=new THREE.Vector3(), qWorld=new THREE.Quaternion();

  function floorHeightAt(x,z){
    const f=window.SVR_PHASE215_FLOOR_HEIGHT||window.SVR_PHASE214_FLOOR_HEIGHT||window.SVR_PHASE213_FLOOR_HEIGHT||window.SVR_PHASE212_FLOOR_HEIGHT;
    if(typeof f==="function"){const y=Number(f(x,z)); if(Number.isFinite(y)) return y;}
    const ax=Math.abs(x);
    if(ax>=9.2&&ax<=19.5&&z<=9.7&&z>=-0.35) return THREE.MathUtils.clamp(((8.65-z)/8.15)*3.42,0,3.42);
    if(z<=-10.0&&z>=-16.4&&ax<=19.8) return 3.42;
    if(ax>=14.6&&ax<=19.8&&z<=7.4&&z>=-13.5) return 3.42;
    return 0;
  }
  ...
  function applyReferenceSpace(){
    ...
    try{const h=-playerYaw*.5; const xf=new XRRigidTransform({x:-playerX,y:-playerY,z:-playerZ},{x:0,y:Math.sin(h),z:0,w:Math.cos(h)}); renderer.xr.setReferenceSpace(baseRefSpace.getOffsetReferenceSpace(xf)); return true;}catch(e){log("[Phase215] ref failed",e?.message||e); return false;}
  }
  ...
  function maintainStanding(){
    ...
    window.SVR_PHASE215_STANDING_LOCK={build:LABEL,standing:!seated,headY:Number(vHead.y.toFixed(2)),targetY:Number(desired.toFixed(2)),playerY:Number(playerY.toFixed(2)),checkedAt:new Date().toISOString()};
  }
  ...
  function snapTurnInPlace(delta){
    ...
    window.SVR_PHASE215_SNAP_TURN={build:LABEL,inPlace:true,checkedAt:new Date().toISOString()};
  }

  const pointer=new THREE.Mesh(new THREE.PlaneGeometry(CONFIG.POINTER_SIZE*1.28,CONFIG.POINTER_SIZE*1.28),new THREE.MeshBasicMaterial({color:0xffffff,transparent:true,alphaTest:.18,opacity:.98,side:THREE.DoubleSide,depthWrite:false,depthTest:false})); pointer.name="PHASE215_TELEPORT_LOGO_TARGET"; pointer.rotation.x=-Math.PI/2; pointer.renderOrder=4000; pointer.visible=false; scene.add(pointer);
  const ring=new THREE.Mesh(new THREE.RingGeometry(.62,1.08,128),new THREE.MeshBasicMaterial({color:PURPLE,transparent:true,opacity:.96,side:THREE.DoubleSide,depthWrite:false,depthTest:false,blending:THREE.AdditiveBlending})); ring.name="PHASE215_PURPLE_GOLD_TARGET_RING"; ring.rotation.x=-Math.PI/2; ring.visible=false; ring.renderOrder=3999; scene.add(ring);
  const beamGeo=new THREE.BufferGeometry(); const beam=new THREE.Points(beamGeo,new THREE.PointsMaterial({color:CYAN,size:.155,sizeAttenuation:true,transparent:true,opacity:.98,depthWrite:false,depthTest:false,blending:THREE.AdditiveBlending})); beam.name="PHASE215_CONTINUOUS_CYAN_PARTICLE_ARCH_BEAM"; beam.frustumCulled=false; beam.visible=false; beam.renderOrder=4001; scene.add(beam);
  const lineGeo=new THREE.BufferGeometry(); const line=new THREE.Line(lineGeo,new THREE.LineBasicMaterial({color:GOLD,transparent:true,opacity:.80,depthWrite:false,depthTest:false,blending:THREE.AdditiveBlending})); line.name="PHASE215_GOLD_FLARE_CENTER_BEAM"; line.visible=false; line.renderOrder=4000; scene.add(line);
  ...
  function headForward(){const xr=renderer.xr.getCamera(camera)||camera,src=xr?.cameras?.[0]||xr;src.getWorldDirection(vForward);vForward.y=0;if(vForward.lengthSq()<1e-5)vForward.set(0,0,-1);return vForward.normalize();}
  function moveControllers(dt){const gR=gp(rightControllerRef),gL=gp(leftControllerRef),r=stick(gR,"right"),l=stick(gL,"left"),now=performance.now(); if(mode){const y=Math.abs(r.y)>.14?r.y:l.y; if(Math.abs(y)>.14)aimDist=THREE.MathUtils.clamp(aimDist + (-y)*8.5*dt,2.0,17.5); return;} if(Math.abs(r.x)>.76&&now>snapCooldownUntil){snapTurnInPlace(Math.sign(r.x)*Math.PI/4);snapCooldownUntil=now+320;} const y=Math.abs(r.y)>.14?r.y:l.y;if(Math.abs(y)<.14)return;const f=headForward().clone(),move=-y,speed=3.65;setPlayerXZ(playerX+f.x*move*speed*dt,playerZ+f.z*move*speed*dt);}
  function controllerAim(proxy){const c=proxy?.userData?.controller||proxy;if(!c)return null;c.updateWorldMatrix?.(true,false);c.getWorldPosition(vOrigin);c.getWorldQuaternion(qWorld);const a=new THREE.Vector3(0,0,-1).applyQuaternion(qWorld).normalize(),bb=new THREE.Vector3(0,0,1).applyQuaternion(qWorld).normalize(),hf=headForward().clone();const af=new THREE.Vector3(a.x,0,a.z);if(af.lengthSq()>0)af.normalize();const bf=new THREE.Vector3(bb.x,0,bb.z);if(bf.lengthSq()>0)bf.normalize();vDir.copy((bf.lengthSq()>0&&bf.dot(hf)>af.dot(hf)+.15)?bf:af);if(vDir.lengthSq()<1e-5)vDir.copy(hf);return clampTarget(vOrigin.x+vDir.x*aimDist,vOrigin.z+vDir.z*aimDist).clone();}
  function handAim(h){const wrist=h?.joints?.wrist,index=h?.joints?.["index-finger-tip"];if(!wrist||!index)return null;wrist.updateWorldMatrix?.(true,false);index.updateWorldMatrix?.(true,false);wrist.getWorldPosition(vWrist);index.getWorldPosition(vIndex);vDir.copy(vIndex).sub(vWrist);vDir.y=0;if(vDir.lengthSq()<.005)vDir.copy(headForward());else vDir.normalize();const hf=headForward().clone();if(vDir.dot(hf)<-.1)vDir.multiplyScalar(-1);vOrigin.copy(vIndex).lerp(vWrist,.35);return clampTarget(vOrigin.x+vDir.x*aimDist,vOrigin.z+vDir.z*aimDist).clone();}
  function teleportByDelta(target){if(!renderer?.xr?.isPresenting||!baseRefSpace)return false;try{const xr=renderer.xr.getCamera(camera);if(!xr)return false;xr.getWorldPosition(vHead);const p=solidXZ(playerX+(target.x-vHead.x),playerZ+(target.z-vHead.z));playerX=p.x;playerZ=p.z;if(!seated)playerY=floorHeightAt(playerX,playerZ);return applyReferenceSpace();}catch(e){log("[Phase215] teleport failed",e?.message||e);return false;}}

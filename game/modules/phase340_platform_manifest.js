...
const ANDROID_FINAL = [
  ...
  'modules/phase367_android_physical_device_viewport_acceptance_lock.js'
];
...
export function validateManifest(platform = detectPlatform()) {
  ...
  if (value === 'android') {
    ...
    ])) forbidden.push('phase367-android-critical-load-order');
    if (!normalized.at(-1)?.endsWith('phase367_android_physical_device_viewport_acceptance_lock.js')) forbidden.push('phase367-android-device-acceptance-not-last');
    if (normalizedDeferred.length) forbidden.push('phase367-android-background-deferred-work');
  }

...
function createCup(root, x, z, color = 0xffffff) {
  ...
  cup.name = "SVR_PHASE113_COFFEE_CUP";
  cup.position.set(x, 1.17, z);
  root.add(cup);
  ...
  lid.name = "SVR_PHASE113_COFFEE_CUP_LID";
  lid.position.set(x, 1.33, z);
  root.add(lid);
  return cup;
}

function createEspressoMachine(root, x, z) {
  const metal = new THREE.MeshStandardMaterial({ color: 0xcbd2dc, roughness: .24, metalness: .55, emissive: 0x101824, emissiveIntensity: .06 });
  const dark = new THREE.MeshStandardMaterial({ color: 0x08090c, roughness: .52, metalness: .18 });
  addBox(root, "SVR_PHASE113_ESPRESSO_MACHINE_BODY", [.54, .42, .36], [x, 1.30, z], metal);
  addBox(root, "SVR_PHASE113_ESPRESSO_MACHINE_FACE", [.42, .24, .045], [x, 1.32, z + .205], dark);
  addBox(root, "SVR_PHASE113_ESPRESSO_MACHINE_TOP", [.62, .08, .42], [x, 1.55, z], metal);
}
...
function hideOldStandPieces(scene) {
  ...
  scene.traverse((obj) => {
    ...
    if (/table|chair|card|chip|bot|dealer|poker|portal|planet|moon|mars|hologram|chakra|plant|rope|pole|carpet|glass|sign|wall|skyline|building|sprite|star|SVR_PHASE113|SVR_PHASE112/i.test(name)) return;
    box.setFromObject(obj);
    box.getSize(size);
    box.getCenter(center);
    const nearOld = Math.abs(center.x - OLD_CENTER.x) < 5.8 && Math.abs(center.z - OLD_CENTER.z) < 4.8 && center.y < 3.4;
    const fixtureLike = size.y < 3.0 && Math.max(size.x, size.z) < 5.5 && Math.max(size.x, size.z) > .30;
    const notFloor = size.y > .045 || center.y > .10;
    if (nearOld && fixtureLike && notFloor) {
      obj.visible = false;
      obj.userData.SVR_PHASE113_OLD_COFFEE_STAND_HIDDEN = true;
      hidden++;
    }
  });
  return hidden;
...
function removePriorCoffeeStands(scene) {
  ["SVR_PHASE112_COFFEE_STAND_RELOCATED", "SVR_PHASE113_COFFEE_STAND_ASSEMBLED"].forEach((name) => {
    const o = scene.getObjectByName(name);
    if (o?.parent) o.parent.remove(o);
  });
}

function yawLocalPositiveZToward(from, to) {
...
export function applyPhase112CoffeeStandMove(scene, { log = console.log } = {}) {
  ...
  addBox(root, "SVR_PHASE113_LEFT_SIDE_SOLID", [.24, .92, 1.34], [-1.54, .62, .02], face);
  addBox(root, "SVR_PHASE113_RIGHT_SIDE_SOLID", [.24, .92, 1.34], [1.54, .62, .02], face);
  addBox(root, "SVR_PHASE113_FRONT_COUNTER_TOP", [3.32, .18, .92], [0, 1.14, .46], white);
  addBox(root, "SVR_PHASE113_SIDE_RETURN_COUNTER", [.86, .18, 1.58], [1.23, 1.14, -.08], white);
  addBox(root, "SVR_PHASE113_BACK_SERVICE_PANEL", [3.05, 1.68, .18], [0, 1.10, -.82], dark);
  addBox(root, "SVR_PHASE113_CANOPY_CONNECTED", [3.42, .18, 1.28], [0, 2.18, -.10], white);
  addBox(root, "SVR_PHASE113_LEFT_CANOPY_POST", [.16, 1.15, .16], [-1.62, 1.58, .55], white);
  addBox(root, "SVR_PHASE113_RIGHT_CANOPY_POST", [.16, 1.15, .16], [1.62, 1.58, .55], white);
  ...
  const light = new THREE.PointLight(0x8ffff0, .66, 6.2, 2.1);
  light.position.set(0, 2.0, .80);
  root.add(light);
  ...
  scene.onBeforeRender = function(...args) {
    ...
    floorMarker.material.opacity = .10 + Math.sin(t * .002) * .035;

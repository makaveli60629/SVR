(function(){
  const BUILD = "VERSION-1.6.3-FORCE-LIVE-SKY-NO-REIKI";
  ...
  const LOCK = {
    ...
    // Any skyline/building object inside this sightline gets capped low.
    corridor: {
      minX: -820,
      maxX: 620,
      minZ: -3300,
  ...
  window.SVR_163_FORCE_LIVE_SKY_LOCK = LOCK;
  ...
  function makePlanet(scene, kind){
    ...
    mesh.name = "SVR_REAL_" + kind.toUpperCase() + "_PHASE_1_6_3_FORCE_HIGH_SKY";
    ...
    mesh.userData.SVR_1_6_3_FORCE_LIVE_SKY = true;
    ...
    const halo = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({
        color: cfg.color,
        transparent: true,
        opacity: kind === "moon" ? 0.18 : 0.12,
        depthWrite: false,
        side: THREE.DoubleSide
      })
    );
    halo.name = "SVR_" + kind.toUpperCase() + "_HALO_PHASE_1_6_3";
  ...
  function planetScore(obj){
    const ud = obj.userData || {};
    const mat = obj.material || {};
    return (ud.SVR_1_6_3_FORCE_LIVE_SKY ? 5000 : 0) +
           (ud.SVR_REAL_PLANET ? 1200 : 0) +
           (mat.map ? 180 : 0) +
           (obj.visible ? 5 : 0);
  }
  ...
  function lockPlanet(scene, kind){
    ...
    list.forEach(obj => {
      ...
      const mat = obj.material || {};
      const ud = obj.userData || {};
      // Hide old fake geometry and old lower duplicates.
      if (!ud.SVR_DO_NOT_REMOVE) {
        obj.visible = false;
        obj.userData = obj.userData || {};
        obj.userData.SVR_1_6_3_HIDDEN_LOWER_DUPLICATE = true;
      }
    });
    ...
    if (keep) {
      ...
      keep.userData.SVR_1_6_3_FORCE_LIVE_SKY = true;
  ...
  function capBuildings(scene){
    ...
    scene.traverse(obj => {
      ...
      obj.userData = obj.userData || {};
      obj.userData.SVR_1_6_3_CAPPED_FOR_HIGH_MOON_MARS = true;
      obj.userData.SVR_VIEWABLE_FROM_LOBBY = true;

      // This is intentionally stronger than earlier phases.
      if (p.y > c.maxY) p.y = c.maxY;
      if (obj.scale && typeof obj.scale.y === "number" && obj.scale.y > c.maxScaleY) obj.scale.y = c.maxScaleY;
    });
  }

  function stampBuildLabels(){
    // Fix visible stale build label only. Does not touch Reiki room.
    document.querySelectorAll("body *").forEach(el => {
      ...
      if (txt.includes("BUILD:") || txt.includes("UPDATE-3.0-PHASE-120")) {
        ...
        el.setAttribute("data-svr163-build-label-fixed","true");
      }
    });

    let badge = document.getElementById("svr163BuildBadge");
    if (!badge) {
      badge = document.createElement("div");
      badge.id = "svr163BuildBadge";
  ...
  function apply(){
    ...
    console.log("[SVR]", BUILD, "active: stale phase label fixed, Moon/Mars forced high, no Reiki room touch");
  }
  ...
  function tick(){
    ...
    ["moon","mars"].forEach(kind => {
      ...
      // Hard-lock each frame so old modules cannot pull them back down.
      obj.visible = true;
  ...
  window.SVR_163_FORCE_LIVE_SKY_NO_REIKI = { build: BUILD, lock: LOCK, apply };
})();

...
export function createPokerDemo({ scene, seats = [], chairRings = [], tableTopY = 0.90, statusCb = () => {}, log = console.log }) {
  ...
  function recordAction(actor, action, amount = 0, stage = '', automatic = false) {
    ...
    paintHistory();
    try { window.dispatchEvent(new CustomEvent('svr_poker_action_log_update', { detail: { build: 'PHASE-229-POWER-DEPLOY-WATCHER-LOCK', latest: clean, actions: actionLog.slice(0, 12), stacks: playerStacks.slice(), pot: current?.pot || 0 } })); } catch (_) {}
  }
  ...
  function recordHistory(entry) {
    ...
    paintHistory();
    try { window.dispatchEvent(new CustomEvent('svr_poker_history_update', { detail: { build: 'PHASE-229-POWER-DEPLOY-WATCHER-LOCK', latest: entry, history: handHistory.slice(0, 8), stacks: playerStacks.slice() } })); } catch (_) {}
  }
  ...
  function emitPokerEvent(name, payload = {}) {
    try { window.dispatchEvent(new CustomEvent(name, { detail: { build: 'PHASE-229-POWER-DEPLOY-WATCHER-LOCK', handNumber, ...payload } })); } catch (_) {}
  }
  ...
  function updateLegalState(stage, callAmount = 0, expiresAt = 0) {
    ...
    paintHistory();
    try { window.dispatchEvent(new CustomEvent('svr_poker_legal_actions_update', { detail: { build: 'PHASE-229-POWER-DEPLOY-WATCHER-LOCK', handNumber, legal: { ...legalState }, decisionAid: { ...decisionAid }, pot: current?.pot || 0, stacks: playerStacks.slice() } })); } catch (_) {}
    publishDecisionAid();
    return legalState;
  }
  ...
  function publishDecisionAid() {
    try {
      window.dispatchEvent(new CustomEvent('svr_poker_decision_aid_update', {
        detail: { build: 'PHASE-229-POWER-DEPLOY-WATCHER-LOCK', handNumber, decisionAid: { ...decisionAid }, legal: { ...legalState }, stacks: playerStacks.slice(), pot: current?.pot || 0 }
      }));
    } catch (_) {}
  }
  ...
  function resolvePlayerTurn(action, automatic = false) {
    ...
    if (action === 'fold') {
      ...
      added = 0;
      try { window.dispatchEvent(new CustomEvent('svr_poker_fold_eligibility_update', { detail: { build: 'PHASE-229-POWER-DEPLOY-WATCHER-LOCK', handNumber, foldedPlayers: getFoldedNames(), seatIndex: PLAYER_INDEX, stage } })); } catch (_) {}
    }
  ...
  function playerAction(action) {
    ...
    if (normalized === 'fold') {
      if (!current.foldedPlayers) current.foldedPlayers = new Set();
      current.foldedPlayers.add(PLAYER_INDEX);
      try { window.dispatchEvent(new CustomEvent('svr_poker_fold_eligibility_update', { detail: { build: 'PHASE-229-POWER-DEPLOY-WATCHER-LOCK', handNumber, foldedPlayers: getFoldedNames(), seatIndex: PLAYER_INDEX, stage: current.stage } })); } catch (_) {}
    }

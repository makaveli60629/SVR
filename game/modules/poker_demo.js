...
export function createPokerDemo({ scene, seats = [], chairRings = [], tableTopY = 0.90, statusCb = () => {}, log = console.log }) {
  ...
  const players = seats.map((seat, i)=>{
    ...
    const center = new THREE.Vector3(seat.x, tableTopY + 0.92, seat.z).addScaledVector(inward, 0.88);
    return {
      anchor: {
        index: i,
        seat,
        ...
        cards: [
          { position: center.clone().addScaledVector(tangent, -0.22), rotationY: 0 },
          { position: center.clone().addScaledVector(tangent, 0.22), rotationY: 0 },
        ]
      },
      chips: [],
  ...
  const boardAnchors = [-0.88, -0.44, 0, 0.44, 0.88].map((x) => ({
    position: new THREE.Vector3(x, tableTopY + 1.14, -0.02),
    rotationY: 0,
  }));
  const burnAnchors = [-0.56, -0.40, -0.24].map((x) => ({
    position: new THREE.Vector3(x, tableTopY + 0.96, -0.50),
    rotationY: 0,
  }));
  ...
  function recordAction(actor, action, amount = 0, stage = '', automatic = false) {
    ...
    paintHistory();
    try { window.dispatchEvent(new CustomEvent('svr_poker_action_log_update', { detail: { build: 'PHASE-252-FORWARD-RESTORE-QUEST-POKER-LOCK', latest: clean, actions: actionLog.slice(0, 12), stacks: playerStacks.slice(), pot: current?.pot || 0 } })); } catch (_) {}
  }
  ...
  function recordHistory(entry) {
    ...
    paintHistory();
    try { window.dispatchEvent(new CustomEvent('svr_poker_history_update', { detail: { build: 'PHASE-252-FORWARD-RESTORE-QUEST-POKER-LOCK', latest: entry, history: handHistory.slice(0, 8), stacks: playerStacks.slice() } })); } catch (_) {}
  }
  ...
  function getCardTexture(card) {
    ...
    const { texture } = makeCanvasLabel({
      ...
      draw(ctx, w, h) {
        ...
        ctx.fillText(rank, 34, 18);
        ctx.font = "bold 122px Georgia, serif";
        ctx.fillText(suit, 38, 158);
        ctx.textAlign = "right";
        ctx.textBaseline = "bottom";
        ctx.fillText(rank, w - 34, h - 154);
        ctx.font = "bold 122px Georgia, serif";
        ctx.fillText(suit, w - 38, h - 24);
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        ctx.font = "bold 292px Georgia, serif";
  ...
  function createCardMesh(card, scale = 1) {
    const mesh = new THREE.Mesh(
      new THREE.PlaneGeometry(0.34 * scale, 0.48 * scale),
  ...
  function emitPokerEvent(name, payload = {}) {
    try { window.dispatchEvent(new CustomEvent(name, { detail: { build: 'PHASE-252-FORWARD-RESTORE-QUEST-POKER-LOCK', handNumber, ...payload } })); } catch (_) {}
  }
  ...
  function updateLegalState(stage, callAmount = 0, expiresAt = 0) {
    ...
    paintHistory();
    try { window.dispatchEvent(new CustomEvent('svr_poker_legal_actions_update', { detail: { build: 'PHASE-252-FORWARD-RESTORE-QUEST-POKER-LOCK', handNumber, legal: { ...legalState }, decisionAid: { ...decisionAid }, pot: current?.pot || 0, stacks: playerStacks.slice() } })); } catch (_) {}
    publishDecisionAid();
    return legalState;
  }
  ...
  function publishDecisionAid() {
    try {
      window.dispatchEvent(new CustomEvent('svr_poker_decision_aid_update', {
        detail: { build: 'PHASE-252-FORWARD-RESTORE-QUEST-POKER-LOCK', handNumber, decisionAid: { ...decisionAid }, legal: { ...legalState }, stacks: playerStacks.slice(), pot: current?.pot || 0 }
      }));
    } catch (_) {}
  }
  ...
  function resolvePlayerTurn(action, automatic = false) {
    ...
    if (action === 'fold') {
      ...
      added = 0;
      try { window.dispatchEvent(new CustomEvent('svr_poker_fold_eligibility_update', { detail: { build: 'PHASE-252-FORWARD-RESTORE-QUEST-POKER-LOCK', handNumber, foldedPlayers: getFoldedNames(), seatIndex: PLAYER_INDEX, stage } })); } catch (_) {}
    }
  ...
  function planHand() {
    ...
    const dealOrder = handPlayers.map((_, offset) => (dealerIndex + 1 - offset + handPlayers.length) % handPlayers.length); // Phase 253 right-to-left visual deal lock
    for (let round = 0; round < 2; round += 1) {
      for (let d = 0; d < dealOrder.length; d += 1) {
        const i = dealOrder[d];
        t += 0.42;
        schedule(t, () => {
          const player = handPlayers[i];
          const card = player.cards[round];
          addCard(card, player.anchor.cards[round], 0.46, 1.08);
          current.stage = "deal";
  ...
  function playerAction(action) {
    ...
    if (normalized === 'fold') {
      if (!current.foldedPlayers) current.foldedPlayers = new Set();
      current.foldedPlayers.add(PLAYER_INDEX);
      try { window.dispatchEvent(new CustomEvent('svr_poker_fold_eligibility_update', { detail: { build: 'PHASE-252-FORWARD-RESTORE-QUEST-POKER-LOCK', handNumber, foldedPlayers: getFoldedNames(), seatIndex: PLAYER_INDEX, stage: current.stage } })); } catch (_) {}
    }

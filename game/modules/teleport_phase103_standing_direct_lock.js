...
const LABEL = "PHASE-103-STANDING-LOCOMOTION-DIRECT-HAND-TELEPORT-LOCK";
const CYAN = 0x7ffcff;
const PURPLE = 0xb55cff;
const GOLD = 0xffd98a;
const STAND_EYE_Y = 1.62;
const MAX_STAND_OFFSET = 2.25;
const MIN_FORWARD_DOT = 0.14;
const SOFT_FORWARD_DOT = 0.48;
...
export function createTeleportRig({ scene, renderer, camera, roomClamp, log=console.log }){
  ...
  window.SVR_PHASE103_STANDING_LOCOMOTION_DIRECT_HAND_LOCK = {
    build: LABEL,
    active: true,
    standingOffsetPreservedWhileMoving: true,
    headForwardStickMovement: true,
    directHandPointingTeleport: true,
    releaseOnlyTeleportCommit: true,
    siteTouched: false,
    checkedAt: new Date().toISOString()
  };
  window.SVR_PHASE101J_LOCOMOTION = window.SVR_PHASE103_STANDING_LOCOMOTION_DIRECT_HAND_LOCK;
  ...
  function standingOffset(){ return seated ? 0 : THREE.MathUtils.clamp(playerY - floorHeightAt(playerX,playerZ), -0.35, MAX_STAND_OFFSET); }
  function applyReferenceSpace(){
    if(!baseRefSpace||!renderer?.xr?.isPresenting) return false;
    try{const h=-playerYaw*.5; const xf=new XRRigidTransform({x:-playerX,y:-playerY,z:-playerZ},{x:0,y:Math.sin(h),z:0,w:Math.cos(h)}); renderer.xr.setReferenceSpace(baseRefSpace.getOffsetReferenceSpace(xf)); return true;}catch(e){log("[103] ref failed",e?.message||e); return false;}
  }
  ...
  function maintainStanding(){
    ...
    if(Math.abs(err)>0.025){playerY += THREE.MathUtils.clamp(err,-0.11,0.11); playerY=THREE.MathUtils.clamp(playerY,floorHeightAt(playerX,playerZ)-.35,floorHeightAt(playerX,playerZ)+MAX_STAND_OFFSET); applyReferenceSpace();}
    window.SVR_PHASE103_STANDING_LOCK={build:LABEL,standing:!seated,headY:Number(vHead.y.toFixed(2)),targetY:Number(desired.toFixed(2)),playerY:Number(playerY.toFixed(2)),standingOffset:Number(standingOffset().toFixed(2)),checkedAt:new Date().toISOString()};
  }
  function setPlayerPose(x,y,z){const p=solidXZ(x,z); const keep=standingOffset(); playerX=p.x; playerZ=p.z; seated=Number.isFinite(y)&&y<-.25; playerY=seated?y:floorHeightAt(playerX,playerZ)+keep; return applyReferenceSpace();}
  function setPlayerXZ(x,z){const keep=standingOffset(); const p=solidXZ(x,z); playerX=p.x; playerZ=p.z; if(!seated) playerY=floorHeightAt(playerX,playerZ)+keep; window.SVR_PHASE103_STICK_STANDING_PRESERVE={build:LABEL,standingOffset:Number(keep.toFixed(2)),playerY:Number(playerY.toFixed(2)),x:Number(playerX.toFixed(2)),z:Number(playerZ.toFixed(2)),checkedAt:new Date().toISOString()}; return applyReferenceSpace();}
  ...
  function snapTurnInPlace(delta){
    const keep=standingOffset();
    if(!renderer?.xr?.isPresenting){playerYaw+=delta;applyReferenceSpace();return;}
    xrHead(vHead); playerYaw += delta; applyReferenceSpace();
    for(let i=0;i<3;i++){xrHead(vHead2); const dx=vHead.x-vHead2.x, dz=vHead.z-vHead2.z; playerX+=dx; playerZ+=dz; if(!seated)playerY=floorHeightAt(playerX,playerZ)+keep; applyReferenceSpace(); if(Math.hypot(dx,dz)<.012)break;}
    window.SVR_PHASE103_SNAP_TURN={build:LABEL,yaw:Number(playerYaw.toFixed(3)),inPlace:true,standingOffset:Number(keep.toFixed(2)),checkedAt:new Date().toISOString()};
  }
  ...
  function updateAura(origin){
    ...
    for(let i=0;i<36;i++){const a=i/36*Math.PI*2+now*(i%2?.55:-.35), r=.10+(i%5)*.011; pts.push(new THREE.Vector3(origin.x+Math.cos(a)*r, origin.y+Math.sin(now+i)*.035, origin.z+Math.sin(a)*r));}
  ...
  function movementForward(){const f=headForward().clone(); if(f.lengthSq()<1e-5) return yawForward().clone(); window.SVR_PHASE103_MOVE_FORWARD_SOURCE={build:LABEL,source:"xr-head-camera",x:Number(f.x.toFixed(3)),z:Number(f.z.toFixed(3)),checkedAt:new Date().toISOString()}; return f;}
  function forwardLockedDirection(candidate, source){
    const hf = headForward().clone();
    const d = new THREE.Vector3(candidate?.x || 0, 0, candidate?.z || 0);
    let corrected = false;
    if(d.lengthSq()<.001){ d.copy(hf); corrected = true; }
    else d.normalize();
    const beforeDot = Number(d.dot(hf).toFixed(3));
    if(beforeDot < MIN_FORWARD_DOT){ d.copy(hf); corrected = true; }
    else if(beforeDot < SOFT_FORWARD_DOT){ d.lerp(hf, .45).normalize(); corrected = true; }
    const afterDot = Number(d.dot(hf).toFixed(3));
    window.SVR_PHASE103_AIM_FORWARD_LOCK={build:LABEL,source,beforeDot,afterDot,corrected,x:Number(d.x.toFixed(3)),z:Number(d.z.toFixed(3)),headX:Number(hf.x.toFixed(3)),headZ:Number(hf.z.toFixed(3)),checkedAt:new Date().toISOString()};
    return d;
  }
  function magneticTarget(t){let best=null,bestD=2.85; scene.traverse(o=>{ if(!o?.userData?.svrMagnetTarget && !/^UPDATE31[DFG]_MAGNET_/i.test(String(o.name||""))) return; const p=new THREE.Vector3(); o.getWorldPosition(p); const d=Math.hypot(p.x-t.x,p.z-t.z); if(d<bestD){bestD=d; best=p;} }); if(!best){ window.SVR_PHASE103_MAGNETIC_LOCK=null; return null; } const target=clampTarget(best.x,best.z).clone(); window.SVR_PHASE103_MAGNETIC_LOCK={build:LABEL,active:true,x:Number(target.x.toFixed(2)),z:Number(target.z.toFixed(2)),dist:Number(bestD.toFixed(2)),checkedAt:new Date().toISOString()}; return target;}
  function aimAxisDistance(gR,gL,r,l,dt){const candidates=[r.y,l.y,r.x,l.x].filter(v=>Math.abs(v)>.14); if(!candidates.length) return; const v=candidates.sort((a,b)=>Math.abs(b)-Math.abs(a))[0]; aimDist=THREE.MathUtils.clamp(aimDist+(-v)*13.5*dt,1.2,26.0); window.SVR_PHASE103_AIM_DISTANCE=Number(aimDist.toFixed(2));}
  function moveControllers(dt){const gR=gp(rightControllerRef),gL=gp(leftControllerRef),r=stick(gR,"right"),l=stick(gL,"left"),now=performance.now(); if(mode){aimAxisDistance(gR,gL,r,l,dt);return true;} if(Math.abs(r.x)>.76&&now>snapCooldownUntil){snapTurnInPlace(Math.sign(r.x)*Math.PI/4);snapCooldownUntil=now+320;} const y=Math.abs(r.y)>.14?r.y:l.y;if(Math.abs(y)<.14)return false; const f=movementForward().clone(),move=-y,speed=4.15; setPlayerXZ(playerX+f.x*move*speed*dt,playerZ+f.z*move*speed*dt); window.SVR_PHASE103_MOVE_VECTOR={build:LABEL,x:Number(f.x.toFixed(3)),z:Number(f.z.toFixed(3)),yaw:Number(playerYaw.toFixed(3)),standingOffset:Number(standingOffset().toFixed(2)),checkedAt:new Date().toISOString()}; return true;}
  function controllerAim(proxy){const c=proxy?.userData?.controller||proxy;if(!c)return null;c.updateWorldMatrix?.(true,false);c.getWorldPosition(vOrigin);c.getWorldQuaternion(qWorld);const forward=new THREE.Vector3(0,0,-1).applyQuaternion(qWorld).normalize(),backward=new THREE.Vector3(0,0,1).applyQuaternion(qWorld).normalize(),hf=headForward().clone();const af=new THREE.Vector3(forward.x,0,forward.z);if(af.lengthSq()>0)af.normalize();const bf=new THREE.Vector3(backward.x,0,backward.z);if(bf.lengthSq()>0)bf.normalize();const chosen=(bf.lengthSq()>0&&bf.dot(hf)>af.dot(hf)+.15)?bf:af;vDir.copy(forwardLockedDirection(chosen,"controller"));const t=clampTarget(vOrigin.x+vDir.x*aimDist,vOrigin.z+vDir.z*aimDist).clone();return magneticTarget(t)||t;}
  function handDirectionFromWrist(wrist,index){
    ...
    if(index){index.updateWorldMatrix?.(true,false); index.getWorldPosition(vIndex); const finger=vIndex.clone().sub(vWrist); finger.y=0; if(finger.lengthSq()>.003){finger.normalize(); window.SVR_PHASE103_DIRECT_HAND_POINTING={build:LABEL,source:"index-tip-minus-wrist",x:Number(finger.x.toFixed(3)),z:Number(finger.z.toFixed(3)),checkedAt:new Date().toISOString()}; return finger;}}
    let best=new THREE.Vector3(0,0,-1).applyQuaternion(qWorld);
    best.y=0; if(best.lengthSq()<.001) best.copy(headForward());
    best.normalize();
    window.SVR_PHASE103_DIRECT_HAND_POINTING={build:LABEL,source:"wrist-forward-fallback",x:Number(best.x.toFixed(3)),z:Number(best.z.toFixed(3)),checkedAt:new Date().toISOString()};
    return best;
  }
  ...
  function teleportByDelta(target){if(!renderer?.xr?.isPresenting||!baseRefSpace)return false;try{const keep=standingOffset();const xr=renderer.xr.getCamera(camera);if(!xr)return false;xr.getWorldPosition(vHead);const p=solidXZ(playerX+(target.x-vHead.x),playerZ+(target.z-vHead.z));playerX=p.x;playerZ=p.z;if(!seated)playerY=floorHeightAt(playerX,playerZ)+keep;window.SVR_PHASE103_LAST_TELEPORT={build:LABEL,targetX:Number(target.x.toFixed(2)),targetZ:Number(target.z.toFixed(2)),playerX:Number(playerX.toFixed(2)),playerZ:Number(playerZ.toFixed(2)),standingOffset:Number(keep.toFixed(2)),gesture:handGestureMode,checkedAt:new Date().toISOString()};return applyReferenceSpace();}catch(e){log("[103] teleport failed",e?.message||e);return false;}}
  ...
  function update({dt=.016,leftHand,rightHand,leftController,rightController,statusCb=()=>{},modeCb=()=>{}}){
    ...
    if(renderer?.xr?.isPresenting&&(leftControllerRef||rightControllerRef)&&!handsAvailable) controllerMoved=moveControllers(dt);
    ...
    window.SVR_PHASE103_SMOKE={build:LABEL,xr:!!renderer?.xr?.isPresenting,handsAvailable,controllerAvailable:!!(leftControllerRef||rightControllerRef),mode,activeMode,handGestureMode,directHandPointing:true,standWhileWalking:true,headForwardMove:true,checkedAt:new Date().toISOString()};
    if(handsAvailable){
      ...
      if(!mode || !active){ hideVisuals(); statusCb("Ready • pinch to aim, release to move"); modeCb("hands direct teleport"); return; }
      ...
      const label = handGestureMode === "pinch" ? "Pinch aim ON • release to move" : "Fist aim ON • release to move";
      statusCb(`${label} • direct hand point`); modeCb(window.SVR_PHASE103_MAGNETIC_LOCK?"magnet target":`${handGestureMode} direct arc`); return;
    }
    if(renderer?.xr?.isPresenting&&(leftControllerRef||rightControllerRef)&&!controllerMoved) moveControllers(dt);
    if(hold>.18&&rc){ if(!mode||active!==rc){mode=true;active=rc;activeMode="controller";triggerHoldStart=now;aimDist=6.2;} }
    if(!mode||!active){hideVisuals();statusCb((leftControllerRef||rightControllerRef)?"Ready • stick walks head-forward, hold trigger to teleport":"Ready • pinch teleport");modeCb("standing lock");return;}
    ...
    const cur=triggerValue(active),held=triggerHoldStart?now-triggerHoldStart:0; if(cur<=.12&&held>70&&now-lastTP>CONFIG.TELEPORT_COOLDOWN_MS){if(teleportByDelta(vSmooth)){lastTP=now;mode=false;active=null;hideVisuals();return;}} if(cur<=.12){mode=false;active=null;hideVisuals();return;} statusCb("Grip TP • release to move"); modeCb(window.SVR_PHASE103_MAGNETIC_LOCK?"magnet target":"controller forward arc");
  }

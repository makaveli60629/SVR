...
export const BUILD = 'PHASE-358-QUEST-FULL-GAME-ACCEPTANCE-SMOOTHNESS-LOCK';
...
function tableAudit() {
  ...
  const fallbackPresent = names.includes('PHASE358_QUEST_TABLE_FALLBACK');
...
async function waitForRuntime(timeoutMs = 120000) {
  ...
  while (performance.now() - started < timeoutMs) {
    ...
    const ready = Boolean(
      ...
      && window.SVR_PHASE358_UPLOADED_TABLE_QA?.().pass === true
    ...
    window.SVR_PHASE358_PROGRESS = {
      stage: 'waiting-runtime',
      elapsedMs: Math.round(performance.now() - started),
      loaded: loaded.length,
      failed: platformState().failed || [],
      uploadedTable: window.SVR_PHASE358_UPLOADED_TABLE_QA?.() || null,
      ready,
...
async function runAcceptance(options = {}) {
  ...
  try {
    ...
    if (!report.runtimeReady) throw new Error(`QUEST_RUNTIME_TIMEOUT:${JSON.stringify(window.SVR_PHASE358_PROGRESS || {})}`);
...
function qa() {
  ...
  const result = {
    ...
    table: tableAudit(),
    uploadedTable: window.SVR_PHASE358_UPLOADED_TABLE_QA?.() || null,
    pokerBoot: window.SVR_PHASE358_POKER_BOOT_QA?.() || null,
    shader: window.SVR_PHASE358_QUEST_SHADER_QA?.() || null,
    platform: {
  ...
  window.SVR_PHASE358_QA_STATE = result;
  return result;
}

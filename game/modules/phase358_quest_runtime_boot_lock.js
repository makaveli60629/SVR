...
export const BUILD = 'PHASE-358-QUEST-RUNTIME-BOOT-LOCK';
...
function installImageSubstitutions() {
  if (!ACTIVE || window.__SVR_PHASE358_IMAGE_SUBSTITUTIONS__) return;
  window.__SVR_PHASE358_IMAGE_SUBSTITUTIONS__ = true;
  ...
  Element.prototype.setAttribute = function phase358SetAttribute(name, value) {
...
function validTable(candidate) {
  ...
  if (candidate.name === 'PHASE358_QUEST_TABLE_FALLBACK') return true;
  try {
...
function findTable() {
  ...
  return [window.SVR_TABLE_AUTHORITY, ...names.map((name) => safeFind(root, name))].find(validTable) || null;
}
...
function ensureTable() {
  ...
  group.name = 'PHASE358_QUEST_TABLE_FALLBACK';
  group.position.set(0, 0, 0.75);
  const rail = new THREE.Mesh(
    new THREE.CylinderGeometry(2.34, 2.34, 0.22, 48),
    new THREE.MeshStandardMaterial({ color: 0x171016, roughness: 0.58, metalness: 0.05 })
  );
  rail.name = 'PHASE358_QUEST_TABLE_RAIL';
  rail.scale.z = 0.64;
  rail.position.y = 0.78;
  const felt = new THREE.Mesh(
    new THREE.CylinderGeometry(2.04, 2.04, 0.055, 48),
    new THREE.MeshStandardMaterial({ color: 0x06351f, roughness: 0.84, metalness: 0.01 })
  );
  felt.name = 'PHASE358_QUEST_TABLE_FELT';
  felt.scale.z = 0.55;
  felt.position.y = 0.925;
  const pedestal = new THREE.Mesh(
    new THREE.CylinderGeometry(0.58, 0.82, 0.72, 32),
...
function drawPot(amount = 0) {
  ...
  g.clearRect(0, 0, w, h);
  roundedRect(g, 8, 8, w - 16, h - 16, 24);
...
function ensurePotDisplay() {
  const root = worldRoot();
  if (!ACTIVE || !root) return null;
  const existing = safeFind(root, 'PHASE358_QUEST_RAISED_TRANSLUCENT_POT_DISPLAY');
  if (existing) {
    potDisplay = existing;
    return existing;
  }
  ...
  potDisplay = new THREE.Sprite(new THREE.SpriteMaterial({
    map: potTexture,
    transparent: true,
    opacity: 0.96,
    depthTest: false,
  ...
  potDisplay.name = 'PHASE358_QUEST_RAISED_TRANSLUCENT_POT_DISPLAY';
  potDisplay.position.set(0, 1.72, 0.72);
  potDisplay.scale.set(1.55, 0.48, 1);
  potDisplay.renderOrder = 9358;
  root.add(potDisplay);
...
function sanitizeScene(limit = 2400) {
  const current = scene();
  if (!current) return 0;
  let repaired = 0;
  stats.boundedSceneNodesInspected = safeWalk(current, (object) => {
...
function govern() {
  ...
  window.SVR_PHASE358_BOOT_STATE = {
    ...stats,
    table: findTable()?.name || null,
    potDisplay: potDisplay?.name || null,
    pot: lastPot
  };
  return window.SVR_PHASE358_BOOT_STATE;
}
...
function qa() {
  ...
  result.pass = result.active
    && Boolean(table)
    && Boolean(result.potDisplay)
    && result.androidRoots === 0
...
if (ACTIVE) {
  ...
  window.SVR_PHASE358_SAFE_WALK = safeWalk;
  window.SVR_PHASE358_SAFE_FIND = safeFind;
  window.SVR_PHASE358_BOOT_GOVERN = govern;
  window.SVR_PHASE358_BOOT_QA = qa;

...
export function seat(){if(window.SVR_QUEST_SEAT_OWNER==='phase446')return true;table=findTable()||table;renderer=window.__SVR_RENDERER__||renderer;camera=window.__SVR_CAMERA__||camera;if(!table)return false;const info=bounds(table),f=forward(table),half=Math.min(info.size.x,info.size.z)/2,target=info.center.clone().addScaledVector(f,half+SEATED_RAIL_GAP);target.y=SEATED_EYE_HEIGHT;const look=info.center.clone();look.y=info.box.max.y-.0254+.05;const ok=setHead(target,look);lockMovement();state.seatLocked=ok;state.installed=ok;state.eyeHeight=SEATED_EYE_HEIGHT;state.checkedAt=new Date().toISOString();return ok}
function frame(){if(!ACTIVE)return;try{lockMovement();const r=rig();if(window.SVR_QUEST_SEAT_OWNER!=='phase446'&&pose&&r?.position){r.position.copy(pose.position);r.rotation.y=pose.yaw}}catch(e){state.lastError=String(e?.message||e)}raf=requestAnimationFrame(frame)}

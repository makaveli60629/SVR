/* PHASE-397-ANDROID-BETTING-ORDER-CHECK-CALL-RAISE-LOCK | PHASE-398-ANDROID-RAISE-SIZING-SMOOTHNESS-LOCK */
import {
...
function recordAction(player,label){
  state.actionTrail ||= [];
  state.actionTrail.push({hand:state.hand,street:state.street,index:player.index,name:player.name,label,tableBet:state.currentBet,lastFullRaiseSize:state.lastFullRaiseSize,at:Date.now()});
...
function performAction(index,type,target=null,automatic=false){
  ...
  if(type==='raise'){
    if(player.raiseLocked){
      player.active=true;tableMessage(`${player.name}: action was not reopened by the short all-in — CALL/FOLD only`);render();return;
    }
    ...
    if(!window.canRaise){
      player.active=true;tableMessage(`${player.name}: full minimum raise unavailable — use CALL/CHECK or ALL IN`);render();return;
    }
    ...
    const result=applyBetIncrease(player,index,player.streetBet);
...
function beginTurn(index){
  ...
  state.turnInterval=setInterval(()=>{
    ...
    if(state.turnSeconds<=0){
      clearTurnTimers();const owed=Math.max(0,state.currentBet-player.streetBet);
      performAction(target,owed>0?'fold':'check',null,true);
    }
  },1000);
...
function resetBettingRound(){
  ...
  state.players.forEach(player=>{
    player.streetBet=0;player.acted=player.folded||player.allIn||player.stack<=0;player.active=false;player.raiseLocked=false;
  });
}
...
function startHand(){
  ...
  tableMessage(`Dealer: ${state.players[state.dealer].name} • SB ${state.players[small].name} ${money(SMALL_BLIND)} • BB ${state.players[big].name} ${money(BIG_BLIND)}`);
...
function bind(){
  ...
  $('#raiseSlider').addEventListener('input',event=>{
    state.raiseTarget=roundToChip(Number(event.target.value));$('#raiseAmount').textContent=`TO ${money(state.raiseTarget)}`;
    $('[data-a="raise"]').textContent=state.currentBet===0?`BET ${money(state.raiseTarget)}`:`RAISE TO ${money(state.raiseTarget)}`;
  });
...
function qa(){
  ...
  const result={
    ...
    minimumRaiseTo:minimumTarget(),lastFullRaiseSize:state.lastFullRaiseSize||BIG_BLIND,

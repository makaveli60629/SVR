export const BUILD = 'PHASE-358-QUEST-POKER-BOOT-ORDER-LOCK';
...
try {
  ...
  if (!state.tableReady) throw new Error('PHASE358_QUEST_TABLE_NOT_READY');
  ...
  if (!state.tablePresentationReady) throw new Error('PHASE358_QUEST_CARDS_NOT_READY');

  await import('./p85_poker_truth_lock.js');
  state.pokerEngineImported = typeof window.SVR_POKER_ACTION === 'function';
  if (!state.pokerEngineImported) throw new Error('PHASE358_QUEST_POKER_ENGINE_NOT_READY');
  ...
  window.SVR_PHASE358_POKER_BOOT_STATE = state;
}

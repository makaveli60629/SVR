...
// Game-side only. Runtime audit synced with Phase 110 Quest performance monitor.
// No website or /site edits.
...
const KNOWN_STALE_PHASES = ["PHASE-85", "PHASE-86", "PHASE-87", "PHASE-88", "PHASE-89", "PHASE-90", "PHASE-91", "PHASE-92", "PHASE-93", "PHASE-94", "PHASE-95", "PHASE-96", "PHASE-97", "PHASE-98", "PHASE-99", "PHASE-100", "PHASE-101", "PHASE-102", "PHASE-103", "PHASE-104", "PHASE-105", "PHASE-106", "PHASE-107", "PHASE-108", "PHASE-109"];
...
function updateVisualLabel(){ const pills = Array.from(document.querySelectorAll(".pill")); const buildPill = pills.find(pill => String(pill.textContent || "").includes("BUILD:")); if (buildPill) buildPill.textContent = `BUILD: ${PHASE}`; if (!String(document.title || "").includes("Phase 110")) document.title = "ScarlettVR Poker • Phase 110 Quest performance regression pass"; }
function runAudit(){
...
function boot(){
  ...
  const loop = () => {
    ...
    let audit;
    try { audit = runAudit(); }
    catch (err){ audit = { phase: PHASE, ok: false, error: err?.message || String(err), timestamp: new Date().toISOString() }; window.SVR_PHASE110_RUNTIME_AUDIT = audit; window.SVR_PHASE105_RUNTIME_AUDIT = audit; window.SVR_PHASE98_RUNTIME_AUDIT = audit; }
    ...
    if (sig !== lastSig){ lastSig = sig; try { console.info("[SVR Phase 110 Runtime Audit]", audit); } catch {} }
    if (runs < 120) setTimeout(loop, 1000);
  };
  setTimeout(loop, 600);
}

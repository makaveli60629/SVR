...
function logoTexture(title, subtitle, color = "#69e8ff", icon = "SVR"){
  ...
  x.clearRect(0,0,768,768); const g = x.createRadialGradient(384,330,40,384,384,360); g.addColorStop(0,color); g.addColorStop(.25,"rgba(255,255,255,.34)"); g.addColorStop(1,"rgba(0,0,0,0)");
  x.fillStyle = g; x.fillRect(0,0,768,768); x.strokeStyle = color; x.lineWidth = 12; x.beginPath(); x.arc(384,306,180,0,Math.PI*2); x.stroke();
  x.fillStyle = "rgba(0,0,0,.55)"; x.beginPath(); x.arc(384,306,154,0,Math.PI*2); x.fill();
...
function addReikiVideo(root){
  const g = new THREE.Group(); g.position.set(20.69,1.64,-5.40); g.rotation.y = THREE.MathUtils.degToRad(168.93); root.add(g);
  ...
  const screen = new THREE.Mesh(new THREE.PlaneGeometry(2.85,1.60), new THREE.MeshBasicMaterial({ map:tex, side:THREE.DoubleSide, toneMapped:false, transparent:true, opacity:.96, depthWrite:false })); screen.position.z = .035; g.add(screen);
  const edge = new THREE.Mesh(new THREE.PlaneGeometry(3.05,1.78), new THREE.MeshBasicMaterial({ color:0x7fffd4, transparent:true, opacity:.075, side:THREE.DoubleSide, depthWrite:false })); edge.position.z = .02; g.add(edge);
...
export function installLobbyVisibilityLock({ scene }){
  const root = new THREE.Group(); root.name = "SVR_Phase98H_Portal_Hologram_Logos"; scene.add(root);
  ...
  addStorefront(root,"Reiki","Main Portal",-5.6,-9.15,"#7fffd4","reiki",{width:3.35}); addStorefront(root,"Reiki","Hologram Here",20.69,-5.40,"#7fffd4","reikiExpanded",{width:4.15,rotationY:THREE.MathUtils.degToRad(168.93)}); addConnector(root,reikiMain,reikiExact);
  ...
  addHoloLogo(root,"Reiki","Main",-5.6,4.15,-8.86,"#7fffd4","REI"); addHoloLogo(root,"Reiki","Hologram",20.69,4.55,-5.18,"#7fffd4","REI",THREE.MathUtils.degToRad(168.93));

...
function makePlanetTexture(kind = "moon"){
  ...
  if(kind === "mars"){
    main.addColorStop(0,"#c97347"); main.addColorStop(.25,"#74301f"); main.addColorStop(.58,"#2d0f0b"); main.addColorStop(1,"#080201");
  } else {
    main.addColorStop(0,"#cfd3dc"); main.addColorStop(.28,"#707784"); main.addColorStop(.60,"#252a34"); main.addColorStop(1,"#07090f");
  }
  ...
  x.globalAlpha = 1;
  const contrast = x.createLinearGradient(0,0,2048,2048); contrast.addColorStop(0,"rgba(255,255,255,.12)"); contrast.addColorStop(.38,"rgba(0,0,0,.02)"); contrast.addColorStop(1,"rgba(0,0,0,.72)"); x.fillStyle = contrast; x.fillRect(0,0,2048,2048);
...
function addGlassHubExpansion(root){
  ...
  const back = new THREE.Mesh(new THREE.PlaneGeometry(10.8,3.15), glassMat); back.position.set(19.25,1.78,-6.72); group.add(back);
  ...
  const edges = [[19.25,3.35,-6.72,10.8,.055,.055],[19.25,.18,-6.72,10.8,.055,.055],[19.25,2.80,-2.88,10.8,.055,.055],[19.25,.18,-2.88,10.8,.055,.055],[13.85,1.62,-4.80,.055,2.85,.055],[24.65,1.62,-4.80,.055,2.85,.055],[13.85,.18,-4.80,.055,.055,3.85],[24.65,.18,-4.80,.055,.055,3.85]];
  ...
  addPanel(group,"Reiki Glass Hub","Expanded wall / volume controls",19.25,3.72,-6.70,COLORS.reiki,6.2,1.15); return group;
}

function addOrb(root,color,x,y,z,scale,kind){
  const g = new THREE.Group(); g.position.set(x,y,z); root.add(g);
  const sphere = new THREE.Mesh(new THREE.SphereGeometry(scale,96,64), new THREE.MeshBasicMaterial({ map:makePlanetTexture(kind), color: kind === "mars" ? 0xb16b50 : 0xb5bbc6 })); g.add(sphere);
  const halo = new THREE.Mesh(new THREE.SphereGeometry(scale*1.15,64,32), new THREE.MeshBasicMaterial({ color, transparent:true, opacity: kind === "mars" ? .035 : .042, side:THREE.BackSide, depthWrite:false })); g.add(halo);
  const light = new THREE.PointLight(color, kind === "mars" ? .38 : .46, 210, 1.6); g.add(light);
  return { group:g, sphere, halo };
}
...
function addReikiVideo(root){
  ...
  const scan = new THREE.Mesh(new THREE.PlaneGeometry(3.15,1.78), new THREE.MeshBasicMaterial({ map:makeScanTexture(), color:0x7fffd4, transparent:true, opacity:.15, side:THREE.DoubleSide, depthWrite:false })); scan.position.z=.052; g.add(scan);
  ...
  panel = new THREE.Mesh(new THREE.PlaneGeometry(4.25,2.18), new THREE.MeshBasicMaterial({ map:makeVolumeTexture(maxVolume,muted), transparent:true, side:THREE.DoubleSide, depthWrite:false, opacity:.98 })); panel.position.set(18.18,2.32,-2.72); panel.rotation.y=THREE.MathUtils.degToRad(180); root.add(panel);
  return { group:g, video, zoneCenter, scan, volumePanel:panel, primeAudio, getState(){ return { videoOn:!video.paused, primed, near:currentVolume>.004, volume:currentVolume, maxVolume, muted }; }, updateAudio(camera,dt=.016){ if(!camera) return; const p=new THREE.Vector3(); camera.getWorldPosition(p); const d=Math.hypot(p.x-zoneCenter.x,p.z-zoneCenter.z); const near=3.75, fade=6.1; let target=0; if(d<fade&&!muted) target=maxVolume*THREE.MathUtils.clamp((fade-d)/(fade-near),0,1); currentVolume=THREE.MathUtils.lerp(currentVolume,target,Math.min(1,dt*2.8)); video.volume=primed?currentVolume:0; video.muted=!primed||muted||currentVolume<.002; scan.material.opacity=.12+Math.sin(performance.now()*.006)*.04; if(video.paused) video.play().catch(()=>{}); } };
}

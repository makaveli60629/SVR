...
const STORE_KEY = 'svr_test_queue_phase252';
...
const SVRTestQueue = {
  ...
  collectSources() {
    const feedback = window.SVR_TESTER_FEEDBACK?.entries || readJson('svr_tester_feedback_phase252', []);
    const bugReports = window.SVR_BUG_REPORTER?.reports || readJson('svr_bug_reports_phase252', []);
  ...
  downloadQueue() {
    ...
    a.download = `svr-test-queue-phase252-${Date.now()}.json`;

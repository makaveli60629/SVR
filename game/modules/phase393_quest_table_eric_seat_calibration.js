...
import {sweep as sweepTable,qa as tableQa,state as tableState} from './phase393_quest_table_surface.js?v=phase395';
import {sweep as sweepEric,qa as ericQa,animate,seatUser,requestSeat,shouldSeat,state as ericState} from './phase393_quest_eric_seat.js?v=phase395';
...
const state={build:BUILD,platform,active:ACTIVE,installed:false,supersededByPhase395:false,lastReason:null,lastError:null,checkedAt:null};
let timer=0,raf=0,renderer=null;
const wait=ms=>new Promise(resolve=>setTimeout(resolve,ms));
async function sweep(reason='interval'){
  if(!ACTIVE)return false;
  if(window.SVR_PHASE395_QUEST_AUTHORITY_ACTIVE){state.supersededByPhase395=true;state.lastReason=`phase395-authority:${reason}`;state.checkedAt=new Date().toISOString();return true}
  try{
...
function frame(time){if(!ACTIVE)return;if(window.SVR_PHASE395_QUEST_AUTHORITY_ACTIVE){state.supersededByPhase395=true;raf=requestAnimationFrame(frame);return}try{animate(time);if(shouldSeat())seatUser('bounded-frame')}catch(error){state.lastError=String(error?.message||error)}raf=requestAnimationFrame(frame)}
function qa(){const table=tableQa(),eric=ericQa(),result={...state,table,eric,pass:Boolean((state.supersededByPhase395||state.installed)&&!state.lastError),checkedAt:new Date().toISOString()};window.SVR_PHASE393_QUEST_STATE=result;return result}

(function(){
  ...
  async function checkJson(url){
    try {
      const r = await fetch(url + (url.includes("?") ? "&" : "?") + "v=phase252-" + Date.now(), { cache: "no-store" });
  ...
  async function run(){
    ...
    checks.push(await checkJson("../deploy-health.json"));
    const phaseOk = checks.some(c => JSON.stringify(c).includes("PHASE-252-FORWARD-RESTORE-QUEST-POKER-LOCK"));

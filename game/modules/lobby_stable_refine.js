...
export const SVR_LOBBY_STABLE_PHASE = "PHASE-139-TELEPORT-SAFE-LOBBY-NO-SKY-LOCK";
...
function makeFloorTexture(){
  return makeCanvasTexture((ctx, size)=>{
    ...
    for (let i = 0; i <= size; i += 96){
      ctx.strokeStyle = "rgba(127,245,199,.34)";
      ctx.lineWidth = 3;
    ...
    ctx.strokeStyle = "rgba(246,226,127,.50)";
    ctx.lineWidth = 10;
    ...
    ctx.fillStyle = "rgba(246,226,127,.74)";
    ctx.fillText("SVR", size/2, size*.496);
    ctx.font = `800 ${Math.floor(size*.028)}px system-ui, Arial`;
    ctx.fillStyle = "rgba(127,245,199,.68)";
    ctx.fillText("VERSION 0.1 • TELEPORT SAFE FLOOR", size/2, size*.545);
  });
}
...
function addBox(parent, name, pos, scale, color, opts = {}){
  const mesh = new THREE.Mesh(new THREE.BoxGeometry(scale.x, scale.y, scale.z), matBasic(color, opts));
  mesh.name = name;
...
function makeWall(parent, { x=0, z=0, rot=0, w=18, h=7.6, title="SVR", sub="", color=COLOR.teal, mode="teal" } = {}){
  const group = new THREE.Group();
  group.name = `SVR_PHASE139_WALL_${title.replace(/\W+/g,"_")}`;
...
function makePortal(parent, { key, title, sub, x, z, rot=0, color=COLOR.teal, mode="teal" } = {}){
  const group = new THREE.Group();
  group.name = `SVR_PHASE139_PORTAL_${key}`;
  group.position.set(x, 2.05, z);
  group.rotation.y = rot;
  parent.add(group);
  const pad = new THREE.Mesh(new THREE.CircleGeometry(1.65, 48), matBasic(color, { transparent:true, opacity:.18, side:THREE.DoubleSide, depthWrite:false }));
  pad.name = `${group.name}_floor_pad`;
  pad.rotation.x = -Math.PI/2;
  pad.position.set(0,-2.00,.68);
  pad.renderOrder = 12;
  group.add(pad);
  const plate = new THREE.Mesh(new THREE.PlaneGeometry(3.15, 1.80), matBasic(0x020610, { transparent:true, opacity:.88, side:THREE.DoubleSide }));
...
function makeTable(parent){
  const group = new THREE.Group();
  group.name = "SVR_PHASE139_SHOW_TABLE_SCORPION_PORTAL";
  group.position.set(0, .74, 0);
  parent.add(group);
  const rail = new THREE.Mesh(new THREE.CylinderGeometry(3.95, 3.95, .28, 80), matBasic(0x24131f));
  rail.name = "SVR_PHASE139_TABLE_RAIL";
  group.add(rail);
  const felt = new THREE.Mesh(new THREE.CylinderGeometry(3.36, 3.36, .30, 80), matBasic(0x0b3d31));
  felt.name = "SVR_PHASE139_TABLE_FELT";
  felt.position.y = .055;
  group.add(felt);
  const passLine = new THREE.Mesh(new THREE.RingGeometry(2.25, 2.31, 80), matBasic(COLOR.gold, { transparent:true, opacity:.28, side:THREE.DoubleSide, depthWrite:false }));
  passLine.name = "SVR_PHASE139_TABLE_PASS_BET_LINE";
  ...
  logo.name = "SVR_PHASE139_TABLE_LOGO";
  ...
  for (let i=0;i<6;i++){
    ...
    chair.name = `SVR_PHASE139_SEAT_${i+1}`;
...
function makeTeleportSafeCeiling(parent, radius){
  const group = new THREE.Group();
  group.name = "SVR_PHASE139_TELEPORT_SAFE_STATIC_CEILING_NO_SKY";
  parent.add(group);
  const cap = new THREE.Mesh(new THREE.PlaneGeometry(radius*2.05, radius*2.05, 1, 1), matBasic(0x020511, { transparent:true, opacity:.72, side:THREE.DoubleSide, depthWrite:false }));
  cap.name = "SVR_PHASE139_STATIC_CEILING_CAP";
  ...
  for (let i=0;i<16;i++){
    ...
    const r = 7 + (i % 4) * 3.2;
    const dot = new THREE.Mesh(new THREE.CircleGeometry(0.055 + (i % 3) * .018, 12), matBasic(i % 2 ? COLOR.teal : COLOR.purple, { transparent:true, opacity:.38, side:THREE.DoubleSide, depthWrite:false }));
    dot.name = `SVR_PHASE139_STATIC_CEILING_DOT_${i}`;
    dot.rotation.x = Math.PI/2;
    dot.position.set(Math.cos(a)*r, 8.70, Math.sin(a)*r);
    group.add(dot);
  }
  return group;
}
...
function makeSpawnPad(parent){
  ...
  pad.name = "SVR_PHASE139_SAFE_SPAWN_PAD_FACE_NORTH";
  pad.rotation.x = -Math.PI/2;
  ...
  arrow.name = "SVR_PHASE139_NORTH_ARROW";
...
export function buildStableLobby(parent, { renderer = null, log = console.log } = {}){
  ...
  root.name = "SVR_PHASE139_TELEPORT_SAFE_NO_SKY_LOBBY";
  parent.add(root);
  ...
  floor.name = "SVR_PHASE139_CORRECT_VISIBLE_LOBBY_FLOOR";
  ...
  floorEdge.name = "SVR_PHASE139_FLOOR_EDGE_LOCK";
  floorEdge.rotation.x = -Math.PI/2;
  floorEdge.position.y = .024;
  root.add(floorEdge);
  makeSpawnPad(root);
  makeTeleportSafeCeiling(root, ROOM_RADIUS);
  makeWall(root, { x:0, z:-23.7, rot:0, w:20, h:8.4, title:"SVR POKER", sub:"VERSION 0.1 • NO SKY FREEZE", color:COLOR.teal, mode:"teal" });
  ...
  window.SVR_PHASE139_STABLE_LOBBY = {
    ...
    skyRemoved:true,
    moon:"disabled for teleport stability",
    mars:"disabled for teleport stability",
    siteTouched:false,
    demo:"SVR-Version 0.1 teleport-safe lobby no-sky build"
  };
  log?.("[Phase139] Teleport-safe no-sky lobby loaded: floor, walls, portals, table, static ceiling, performance lock.");
  return { root, roomClamp: ROOM_RADIUS - 1.5, seats, tableCenter, joinRadius: 6.4, previewOrbitRadius: 20, sceneTargets, update: root.userData._tickWorld };
}

...
const BUILD = "LOBBY-ORG-1-2-REIKI-LUXURY-CLEANUP-CAROUSEL-VIDEO-GATE";
...
function setupVideoPosition(root) {
  ...
  const found = [];
  for (const [name, pos, rot] of videoParts) { const obj = root.getObjectByName(name); if (!obj) continue; obj.position.set(pos[0], pos[1], pos[2]); obj.rotation.set(rot[0], rot[1], rot[2]); obj.renderOrder = Math.max(obj.renderOrder || 0, 365); found.push(obj); }
  return found;
}
function pauseVideo(reset = false) {
  const video = window.SVR_REIKI_HOLOGRAM_VIDEO?.element || document.getElementById("svr-reiki-hologram-video");
  if (!video) return false;
  video.muted = true;
  video.volume = 0;
  video.pause?.();
  if (reset) { try { video.currentTime = 0; } catch (_) {} }
  window.SVR_REIKI_VIDEO_GATE_STATE = "paused-hidden";
  return true;
}
function playVideoMuted() {
  ...
  video.volume = 0;
  video.play?.().catch?.(() => {});
  window.SVR_REIKI_VIDEO_GATE_STATE = "playing-muted-visible";
  return true;
}
function patchCarousel(root) {
  ...
  const cards = [
    { type: "about", title: "WELCOME TO REIKI", lines: ["Holistic wellness preview", "Swipe/slide for the interview", "Presentation module only", "AWAITING APPROVAL"] },
    { type: "video", title: "INTERVIEW VIDEO", lines: ["Hologram media console", "Visible only on this card", "Action button enables sound", "AWAITING APPROVAL"] },
  ...
  function setCard(i) {
    ...
    if (!isVideo) { cardMesh.material = panelMat(makeTexture(card.title, card.lines, { border: card.type === "portal" ? "rgba(0,204,255,.90)" : "rgba(0,255,204,.82)" }), 0.97); cardMesh.material.needsUpdate = true; }
    window.SVR_REIKI_CAROUSEL_12_ACTIVE = { index: active, card };
  }
  function activateCard() {
    const card = cards[active];
    if (card.type === "video") { window.SVR_REIKI_HOLOGRAM_VIDEO?.play?.(); return; }
    if (card.type === "shop") { window.open("/site/reiki-about.html#shop", "_blank", "noopener,noreferrer"); return; }
  ...
  window.SVR_RICI_UPDATE_101_CAROUSEL = { next: () => setCard(active + 1), prev: () => setCard(active - 1), activate: activateCard, setCard, getActiveIndex: () => active, getActiveCard: () => cards[active], cards, patchedBy: BUILD, videoGate: true };
  setCard(0);
  pauseVideo(true);
  return true;
}
export function applyReikiLuxuryCleanup12(scene, { log = console.log } = {}) {
  ...
  window.SVR_REIKI_LUXURY_CLEANUP_12 = { build: BUILD, hiddenEntryObjects: hidden, carouselPatched, aboutFirst: true, videoSecond: true, videoGatedUntilSecondCard: true, carpetClear: true, welcomeFloorRemoved: true, thresholdRemoved: true };
  log?.("Reiki luxury cleanup 1.2 video gate loaded", window.SVR_REIKI_LUXURY_CLEANUP_12);
  return lock;
}

...
(function(){
  ...
  async function run(){
    ...
    try {
      ...
      add('boot-script-cache', scripts.includes('boot.js?v=phase226'), scripts || 'no scripts found');
      const version = await fetchJson('./version.json?v=phase226-' + Date.now());
      add('version-json-marker', version && version.build === BUILD && Number(version.phase) === 211, JSON.stringify(version));
      const gameHealth = await fetchJson('./deploy-health.json?v=phase226-' + Date.now());

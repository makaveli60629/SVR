/* SVR Poker Phase 211 - Full Marker Health Lock
...
(function(){
  ...
  async function run(){
    ...
    try {
      ...
      add('title-marker', title.includes('Phase 211') || title.includes(BUILD), title);
      add('hud-build-marker', hudText.includes(BUILD), 'HUD/body contains current build marker');
      add('boot-script-cache', scripts.includes('boot.js?v=phase211'), scripts || 'no scripts found');
      const version = await fetchJson('./version.json?v=phase211-' + Date.now());
      add('version-json-marker', version && version.build === BUILD && Number(version.phase) === 211, JSON.stringify(version));
      const gameHealth = await fetchJson('./deploy-health.json?v=phase211-' + Date.now());

...
function ensureUi(){
  if(state.installed)return true;const turn=$('.turn-strip'),footer=$('.footer');if(!turn||!footer)return false;
  turn.insertAdjacentHTML('afterend','<section id="phase404DecisionStrip" class="phase404-decision-strip" aria-label="Current poker decision information"><span><small>POT</small><b data-pot>$0</b></span><span><small>CALL</small><b data-call>FREE</b></span><span><small>POT ODDS</small><b data-odds>0%</b></span><span><small>STACK</small><b data-stack>$0</b></span><span><small>MIN RAISE</small><b data-min>-</b></span></section>');
...
function qa(){const g=game();return{build:BUILD,installed:state.installed,decisionStrip:Boolean($('#phase404DecisionStrip')),allInDoubleTapGuard:true,allInConfirmMs:ALL_IN_CONFIRM_MS,sessionReport:Boolean($('#phase404SessionSheet')),copyTestReport:true,wakeLockSupported:state.wakeLockSupported,wakeLockHeld:Boolean(state.wakeLock),engineBuild:g?.engineBuild||null,phase403EnginePreserved:g?.engineBuild==='PHASE-403-ANDROID-POKER-ENGINE-RELIABILITY-LOCK',lastError:state.lastError,pass:Boolean(state.installed&&$('#phase404DecisionStrip')&&$('#phase404SessionSheet')&&!state.lastError),checkedAt:new Date().toISOString()}}

...
const LABEL = 'PHASE-158-ACTUAL-FBX-TABLE-SCALE-ORIENTATION-LOCK';
const ROOT = 'PHASE158_ACTUAL_FBX_TABLE_SCALE_ROOT';
...
const TARGET_TABLE_LENGTH = 4.55;
const TARGET_TABLE_DEPTH = 2.35;
const TABLE_TOP_Y = 0.88;
...
function removeGeometricFallback(root){
  const kill=[];
  root?.traverse?.(o=>{
    const name=String(o.name||'');
    if(/^PHASE157_STABLE_|^PHASE156_TABLE2_|^PHASE155_ENHANCED_REAL_TABLE_FALLBACK|^PHASE155_RESTORED_ASSET_TABLE/.test(name)) kill.push(o);
  });
...
function addSeats(root){
  if(root.getObjectByName('PHASE158_SEAT_GUIDE_ROOT')) return;
  const seats=new THREE.Group(); seats.name='PHASE158_SEAT_GUIDE_ROOT'; root.add(seats);
  [[0,STOOL_RING_RADIUS,true],[-2.55,1.75,false],[-2.55,-1.35,false],[0,-3.05,false],[2.55,-1.35,false],[2.55,1.75,false]].forEach((s,i)=>addSeat(seats,s[2]?'PHASE158_OPEN_PLAYER_STOOL':'PHASE158_BOT_STOOL_'+i,s[0],s[1],s[2]));
}
...
function normalizeToStools(obj){
  ...
  let b=bounds(obj);
  if(b.size.y > Math.max(b.size.x,b.size.z)*1.18){
    obj.rotation.x = -Math.PI/2;
    obj.updateMatrixWorld(true);
    b=bounds(obj);
  }

  if(b.size.z > b.size.x * 1.35){
  ...
  const scale = THREE.MathUtils.clamp(Math.min(scaleX, scaleZ), 0.001, 8);
  obj.scale.multiplyScalar(scale);
  obj.updateMatrixWorld(true);
  b=bounds(obj);

  obj.position.x -= b.center.x;
  obj.position.z -= b.center.z;
  obj.position.y += TABLE_TOP_Y - b.box.min.y;
  obj.updateMatrixWorld(true);
  b=bounds(obj);
...
function install(scene=window.__SVR_SCENE__){
  ...
  hideOld(host);
  loadFbxInto(root);
  window.SVR_PHASE158_ACTUAL_FBX_TABLE_SCALE_ORIENTATION_LOCK={
    build:LABEL,
    active:true,
    noBlink:true,
  ...
  window.SVR_PHASE156_TABLE2_STOOL_TEXTURE_LOCK=window.SVR_PHASE158_ACTUAL_FBX_TABLE_SCALE_ORIENTATION_LOCK;
  window.SVR_RUN_PHASE156_TABLE2_AUDIT=()=>window.SVR_PHASE158_ACTUAL_FBX_TABLE_SCALE_ORIENTATION_LOCK;
  window.SVR_RUN_PHASE157_TABLE_AUDIT=()=>window.SVR_PHASE158_ACTUAL_FBX_TABLE_SCALE_ORIENTATION_LOCK;
  window.SVR_RUN_PHASE158_TABLE_AUDIT=()=>window.SVR_PHASE158_ACTUAL_FBX_TABLE_SCALE_ORIENTATION_LOCK;
  return true;
}

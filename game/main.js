...
import { buildSkylineRoom } from "./modules/world_skyline.js?v=phase173-4-legend-hall-boot-fix";
...
function runPokerAction(action){
  if (action === 'next') {
    ...
    setStatus('Poker: next hand requested', { force: true });
    return true;
  }
...
$pokerActionButtons.forEach((btn)=>{
  btn.addEventListener('click', ()=>{
    const action = btn.dataset.action;
    if (action) runPokerAction(action);
  });
});
...
window.addEventListener("keydown", async (e)=>{
  ...
  if (e.code === "KeyF") runPokerAction("fold");
  if (e.code === "KeyC") runPokerAction("call");
  if (e.code === "KeyR") runPokerAction("raise");
  if (e.code === "KeyX") runPokerAction("check");
  if (e.code === "KeyA") runPokerAction("allin");
  if (e.code === "KeyH") runPokerAction("next");
...
const watch = createWristWatch({
  ...
  actions: {
    ...
    pokerFold: ()=>runPokerAction("fold"),
    pokerCheck: ()=>runPokerAction("check"),
    pokerCall: ()=>runPokerAction("call"),
    pokerRaise: ()=>runPokerAction("raise"),
    pokerAllIn: ()=>runPokerAction("allin")
  }
});
...
setStatus(AUTOCAM ? "Live preview ready" : "Ready • PHASE-173-4-LEGEND-HALL-BOOT-FIX. Winner proof active. Matrix billboard and Legend Hall boot hotfixes applied. Keys: F fold, X check, C call, R raise, A all-in, H next hand.", { force: true });

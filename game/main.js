...
function log(...args){ const line = args.map(a => typeof a === "string" ? a : JSON.stringify(a, null, 2)).join(" "); $log.textContent += line + "\n"; $log.scrollTop = $log.scrollHeight; }
...
function distanceToTarget(key){ const rec = sceneTargets?.[key]; if (!rec?.pos) return Infinity; const p = currentHeadXZ(); return Math.hypot(p.x - rec.pos.x, p.z - rec.pos.z); }
function isInReikiArea(){ return distanceToTarget("reiki") <= 3.6 || distanceToTarget("reikiRoom") <= 4.2; }
function gotoScene(key){ if (key === "reikiVideoPortal") return openReikiVideoPortal(); const rec = sceneTargets?.[key]; if (!rec?.pos) return false; movePlayerToSpot(rec.pos, rec.look || null); setStatus(`Quick jump: ${key}`, { force: true }); return true; }
function openReikiVideoPortal(){ if (!isInReikiArea()){ setStatus("Reiki hologram stays paused and only activates from the Reiki storefront. Jump to Reiki first.", { force: true }); return false; } window.location.href = "./reiki-video-portal.html?v=phase130-reiki-one-display&zone=reiki"; return true; }
function openStorePortal(){ const url = "https://svrpoker.com/site/store.html"; window.open(url, "_blank", "noopener,noreferrer"); setStatus("SVR Store portal opened in a safe browser tab.", { force: true }); return true; }
function createStoreWebPortal(){
  const rec = sceneTargets?.store || sceneTargets?.sponsor; if (!rec?.pos) return null;
  const group = new THREE.Group(); group.name = "SVR Store Web Portal"; group.position.set(rec.pos.x + 0.35, 1.7, rec.pos.z - 0.35); if (rec.look) group.lookAt(rec.look.x, 1.45, rec.look.z);
  ...
  const pane = new THREE.Mesh(new THREE.PlaneGeometry(2.1, 1.12), glowMat); pane.userData.href = "https://svrpoker.com/site/store.html"; group.add(pane);
  const frame = new THREE.Mesh(new THREE.TorusGeometry(0.82, 0.018, 12, 112), new THREE.MeshBasicMaterial({ color: 0x9b5cff, transparent: true, opacity: 0.66, side: THREE.DoubleSide })); frame.scale.set(1.42, 0.72, 1); frame.position.z = 0.025; group.add(frame);
  const canvas = document.createElement("canvas"); canvas.width = 1200; canvas.height = 650; const ctx = canvas.getContext("2d");
  const grad = ctx.createLinearGradient(0,0,1200,650); grad.addColorStop(0, "rgba(3,12,20,.95)"); grad.addColorStop(1, "rgba(18,8,34,.96)"); ctx.fillStyle = grad; ctx.fillRect(0,0,1200,650);
  ctx.strokeStyle = "rgba(94,247,255,.94)"; ctx.lineWidth = 14; ctx.strokeRect(28,28,1144,594); ctx.textAlign = "center"; ctx.textBaseline = "middle"; ctx.fillStyle = "#ffffff"; ctx.font = "bold 76px system-ui, Arial"; ctx.fillText("SVR STORE PORTAL", 600, 150); ctx.fillStyle = "#9ffcff"; ctx.font = "bold 44px system-ui, Arial"; ctx.fillText("VR-friendly web store preview", 600, 245); ctx.fillStyle = "#d9d4ff"; ctx.font = "34px system-ui, Arial"; ctx.fillText("https://svrpoker.com/site/store.html", 600, 340); ctx.fillStyle = "#ffdd88"; ctx.font = "bold 34px system-ui, Arial"; ctx.fillText("Click / tap to open", 600, 445); ctx.fillStyle = "#bffcff"; ctx.font = "28px system-ui, Arial"; ctx.fillText("Store opens outside the game so the lobby stays smooth", 600, 505);
  const tex = new THREE.CanvasTexture(canvas); tex.colorSpace = THREE.SRGBColorSpace; const panel = new THREE.Mesh(new THREE.PlaneGeometry(2.0, 1.08), new THREE.MeshBasicMaterial({ map: tex, transparent: true, side: THREE.DoubleSide })); panel.position.z = 0.035; group.add(panel); scene.add(group);
...
window.addEventListener("keydown", async (e)=>{
  ...
  if (e.code === "Digit1") gotoScene("lobby"); if (e.code === "Digit2") gotoScene("table"); if (e.code === "Digit3") gotoScene("seat"); if (e.code === "Digit4") gotoScene("reiki"); if (e.code === "Digit5") gotoScene("pga"); if (e.code === "Digit6") gotoScene("legends"); if (e.code === "Digit7") gotoScene("sponsor"); if (e.code === "Digit8") gotoScene("scorpion"); if (e.code === "Digit9") openReikiVideoPortal(); if (e.code === "Digit0") gotoScene("store"); if (e.code === "KeyO") openStorePortal();
});
...
const watch = createWristWatch({
  ...
  actions: { toggleAudio: ()=>audio.toggle(), nextTrack: ()=>audio.next(), joinTable, leaveTable, toggleTeleport: ()=>tp.toggleMode(), goLobby: ()=>gotoScene("lobby"), goTable: ()=>gotoScene("table"), goSeat: ()=>gotoScene("seat"), goReiki: ()=>gotoScene("reiki"), goPga: ()=>gotoScene("pga"), goStore: ()=>gotoScene("store"), openStore: ()=>openStorePortal(), goLegend: ()=>gotoScene("legends"), goSponsor: ()=>gotoScene("sponsor"), goScorpion: ()=>gotoScene("scorpion"), goReikiRoom: ()=>openReikiVideoPortal() }
});
...
renderer.xr.addEventListener("sessionstart", async ()=>{ perf.onXRSessionStart(); setHudVisible(false); document.getElementById("sceneNav")?.style.setProperty("display","none"); await tp.onSessionStart(); });

...
if (questRelease.phase !== 380) errors.push('Quest release phase mismatch');
if (questRelease.build !== 'PHASE-380-ORIGINAL-UPLOADED-TABLE-AUTHORITY-LOCK') errors.push('Quest release build mismatch');
...
if (questRelease.browserAcceptance?.uploadedTable !== 'PHASE380_ORIGINAL_UPLOADED_TABLE_GLB_AUTHORITY') errors.push('Phase 380 original table record missing');
if (questRelease.browserAcceptance?.proceduralFallbackEmergencyOnly !== true) errors.push('Emergency fallback policy missing');
if (questRelease.sessionContract?.teleportLockedWhileSeated !== true) errors.push('Seated teleport policy regressed');
...
if (androidRelease.apkVersionName !== '0.1.0-rc2' || androidRelease.apkVersionCode !== 2 || androidRelease.manualUpdateOnly !== true || androidRelease.showUpdatePrompt !== false) errors.push('Android APK policy regressed');
if (webManifest.apk_version_name !== '0.1.0-rc2' || webManifest.apk_version_code !== 2 || webManifest.manual_update_only !== true || webManifest.show_update_prompt !== false) errors.push('Web APK policy regressed');

...
(async () => {
  ...
  vm.runInContext(`
    ...
    var createPhase148QuestPerfPass=()=>({lockSceneForQuest(){},onXRSessionStart(){},optionalTickAllowed:()=>false});
  ...
  c.inputs.leftController = c.hand;
  c.window.testFrame();
  assert.equal(watch.visible, true, 'controller proxy restores the watch');
  ...
  vm.runInContext(`
    ...
    var felt=new THREE.Mesh(new THREE.BoxGeometry(2,.04,1));felt.position.y=.7;group.add(felt);
    var runtime={table:{group,nativeFeltRecords:[{mesh:felt}]},dealer:{}};
  ...
  `, geo);
  assert.equal(geo.result.removed, 1, 'unnamed broad hovering cover must be found geometrically');
  ...
  console.log('Quest main-loop, watch tracking/input, table-only world, geometric clearance and platform regressions passed.');

...
for (const [token, label] of [
  ['phase349_multiplayer_presence_seat_reconnect_lock.js', 'presence-module'],
  ['phase349_presence_gameplay_seat_bridge.js', 'seat-bridge-module'],
  ['const SHARED_SOCIAL = [', 'shared-presence-tail'],
  ['...ACCOUNT_ACTIVITY', 'shared-account-tail'],
  ...
  ['phase367-android-background-deferred-work', 'android-zero-background-validator'],
...
for (const [token, label] of [
  ["app.post('/api/presence/join'", 'server-join'],
  ["app.post('/api/presence/heartbeat'", 'server-heartbeat'],
  ["app.get('/api/presence/room/:roomId'", 'server-list'],
  ["app.post('/api/presence/seat/claim'", 'server-seat-claim'],
  ["app.post('/api/presence/seat/release'", 'server-seat-release'],
  ["app.post('/api/presence/leave'", 'server-leave'],
  ['await transaction.begin(sql.ISOLATION_LEVEL.SERIALIZABLE)', 'transactional-seat-claim'],
  ['jwt.verify', 'authenticated-presence']
]) requireText(server, token, label);
for (const [token, label] of [
  ['CREATE TABLE dbo.PlayerPresence', 'presence-table'],
  ['PRIMARY KEY (RoomId, PlayerId)', 'one-player-per-room'],
  ['CREATE TABLE dbo.PlayerSeatLeases', 'seat-lease-table'],
  ['PRIMARY KEY(RoomId,SeatId)', 'one-owner-per-seat'],
  ['CHECK(SeatId BETWEEN 0 AND 5)', 'six-seat-limit']
]) requireText(schema, token, label);
...
console.log(JSON.stringify({
  ...
  androidLoadPolicy: 'zero-deferred-social-work-during-table-play',

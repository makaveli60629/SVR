...
for (const token of [
  'PHASE350_CAMERA3_LIGHTING_ROOT', 'new THREE.HemisphereLight', 'new THREE.AmbientLight',
  'new THREE.DirectionalLight', 'new THREE.PointLight', 'toneMappingExposure = 1.22',
  'shadowMap.enabled = false', 'SVR_PHASE350_CAMERA3_QA'
]) requireText(camera3, token, `camera3-${token}`);

for (const token of [
  "const AUTHORITY_ID = 'svr347Root'", '#svr326Root', '#svr343Hud',
  'new MutationObserver', 'node.remove', 'SVR_PHASE350_ANDROID_CONTROLLER_QA',
  'visibleLegacyRoots === 0', 'externalSticks === 0'
...
requireText(platform, 'const order = [', 'android-order-validator');
requireText(platform, 'order.at(-1) !== normalized.length - 1', 'android-dedupe-final-validator');
...
requireText(platform, 'deferredManifestFor', 'deferred-export');

const androidFinalBlock = platform.slice(platform.indexOf('const ANDROID_FINAL ='), platform.indexOf('const ANDROID_DEFERRED ='));
if (!androidFinalBlock.trim().endsWith('];')) errors.push('android-final-array');
...
if (!(driverIndex >= 0 && dedupeIndex > driverIndex)) errors.push('android-dedupe-not-last-after-driver');

...
const successorPath = 'game/modules/phase367_android_physical_device_viewport_acceptance_lock.js';
...
for (const token of ['visualViewport', 'safe-area-inset-bottom', 'SVR_PHASE365_STABILIZE_SEAT', 'SVR_PHASE367_DEVICE_QA']) {
  requireText(successorSource, token, `phase367-preserves-phase365-${token}`);
}

requireText(platformManifest, "export const VERSION = 'phase367'", 'platform-version');
requireText(platformManifest, "'modules/phase365_android_seated_ux_branding_gyro_alignment_lock.js',\n  'modules/phase367_android_physical_device_viewport_acceptance_lock.js'", 'phase365-before-phase367');
requireText(platformManifest, 'phase367-android-device-acceptance-not-last', 'phase367-last-validator');
...
const htmlPhase367 = androidHtml.indexOf('phase367_android_physical_device_viewport_acceptance_lock.js?v=phase367');
const htmlSettlement = androidHtml.indexOf('phase363_android_settlement_lobby_consistency_lock.js?v=phase367');
if (htmlPhase365 < 0 || htmlPhase367 < 0 || htmlSettlement < 0 || htmlPhase365 < htmlSettlement || htmlPhase367 < htmlPhase365) errors.push('android-html-phase365-phase367-order');
requireText(androidHtml, "window.SVR_ANDROID_BRAND_SLOT={id:'svr',name:'SVR POKER'", 'android-default-brand-slot');
requireText(androidHtml, 'data-build="PHASE-367-ANDROID-PHYSICAL-DEVICE-VIEWPORT-ACCEPTANCE-LOCK"', 'android-successor-build-label');
...
if (androidRelease.currentGameBuild !== 'PHASE-367-ANDROID-PHYSICAL-DEVICE-VIEWPORT-ACCEPTANCE-LOCK') errors.push('release-build');

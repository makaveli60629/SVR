...
renderer.xr.addEventListener('sessionstart',()=>{controls.enabled=false;table.toggleGuides(false);document.body.classList.add('preview');if(dealer.loaded)groundEric(false);syncWatch();refreshStatus()});
...
click('frontCamBtn',()=>setCameraPreset('front'));click('handsCamBtn',()=>setCameraPreset('hands'));click('tableCamBtn',()=>setCameraPreset('table'));click('focusEricBtn',focusEric);click('focusEricBtn2',focusEric);click('groundEricBtn',()=>groundEric(true));click('groundEricBtn2',()=>groundEric(true));click('idleBtn',()=>{dealer.setMode('idle');syncWatch();refreshStatus()});click('dealOnceBtn',()=>{dealer.setMode('deal-once');syncWatch();refreshStatus()});click('dealLoopBtn',()=>{dealer.setMode('deal-loop');syncWatch();refreshStatus()});click('pauseBtn',()=>{dealer.togglePause();syncWatch();refreshStatus()});click('diagBtn',()=>{table.toggleGuides();syncWatch();refreshStatus()});click('resetEricBtn',()=>{const p=dealer.resetVisible();writeDealerInputs(p);debugEric=false;dealer.setDebugMaterial(false);groundEric(false);focusEric();refreshPreset();refreshStatus()});click('debugEricBtn',e=>{debugEric=!debugEric;dealer.setDebugMaterial(debugEric);e.currentTarget.classList.toggle('active',debugEric);refreshStatus()});click('collapseHudBtn',e=>{hud.classList.toggle('compact');e.currentTarget.textContent=hud.classList.contains('compact')?'Expand':'Compact'});click('hideHudBtn',()=>{hud.classList.add('hidden');document.getElementById('showHudBtn').style.display='block'});click('showHudBtn',()=>window.SVR_OPEN_DEALER_ADJUSTMENTS?.());click('previewBtn',()=>{document.body.classList.add('preview');table.toggleGuides(false);syncWatch();refreshStatus()});

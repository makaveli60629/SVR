...
function enforce(reason='manual-scale-lock'){
  ...
  const scale=Number(dealer.params?.scale||0);
  const positionOk=Math.abs(Number(dealer.params?.x)-APPROVED.x)<1e-7&&Math.abs(Number(dealer.params?.z)-APPROVED.z)<1e-7;
  if(Math.abs(scale-APPROVED.scale)<1e-7&&positionOk){updateScaleControl(APPROVED.scale);return {build:BUILD,reason,scale,unchanged:true}}
  enforcing=true;
  try{
...
let attempts=0;
const timer=setInterval(()=>{attempts+=1;if(attach()||attempts>80)clearInterval(timer)},100);

...
## Fixes
- Updates `index.html` script query from stale phase values to `boot.js?v=phase222`.
...
- Adds cache-busted `enterprise_bridge_phase222.js`.
- Keeps public Matrix launch page untouched.

## Test
Open `/game/?v=phase222-bootcache`, then verify the HUD says `PHASE-222-POST-DEPLOY-CHECKLIST-LOCK` and the game does not remain stuck on Booting.

...
## Fixes
- Updates `index.html` script query from stale phase values to `boot.js?v=phase227`.
...
- Adds cache-busted `enterprise_bridge_phase227.js`.
- Keeps public Matrix launch page untouched.

## Test
Open `/game/?v=phase227-bootcache`, then verify the HUD says `PHASE-227-PILOT-ISSUE-TEMPLATE-LOCK` and the game does not remain stuck on Booting.

...
## Fixes
- Updates `index.html` script query from stale phase values to `boot.js?v=phase226`.
...
- Adds cache-busted `enterprise_bridge_phase226.js`.
- Keeps public Matrix launch page untouched.

## Test
Open `/game/?v=phase226-bootcache`, then verify the HUD says `PHASE-226-PILOT-READY-SUMMARY-LOCK` and the game does not remain stuck on Booting.

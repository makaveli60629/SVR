...
## Fixes
- Updates `index.html` script query from stale phase values to `boot.js?v=phase223`.
...
- Adds cache-busted `enterprise_bridge_phase223.js`.
- Keeps public Matrix launch page untouched.

## Test
Open `/game/?v=phase223-bootcache`, then verify the HUD says `PHASE-223-TESTER-LAUNCH-CARD-LOCK` and the game does not remain stuck on Booting.

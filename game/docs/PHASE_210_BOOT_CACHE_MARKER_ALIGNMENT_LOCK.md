...
## Fixes
- Updates `index.html` script query from stale phase values to `boot.js?v=phase233`.
...
- Adds cache-busted `enterprise_bridge_phase233.js`.
- Keeps public Matrix launch page untouched.

## Test
Open `/game/?v=phase233-bootcache`, then verify the HUD says `PHASE-233-OPTIONAL-MODULE-LOADER-LOCK` and the game does not remain stuck on Booting.

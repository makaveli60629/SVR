...
## Fixes
- Updates `index.html` script query from stale phase values to `boot.js?v=phase238`.
...
- Adds cache-busted `enterprise_bridge_phase238.js`.
- Keeps public Matrix launch page untouched.

## Test
Open `/game/?v=phase238-bootcache`, then verify the HUD says `PHASE-238-HAND-TELEPORT-PINCH-DESTINATION-LOCK` and the game does not remain stuck on Booting.

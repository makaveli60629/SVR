...
## Fixes
- Updates `index.html` script query from stale phase values to `boot.js?v=phase244`.
...
- Adds cache-busted `enterprise_bridge_phase244.js`.
- Keeps public Matrix launch page untouched.

## Test
Open `/game/?v=phase244-bootcache`, then verify the HUD says `PHASE-244-MAIN-RUNTIME-CATCH-FIX-LOCK` and the game does not remain stuck on Booting.

...
## Fixes
- Updates `index.html` script query from stale phase values to `boot.js?v=phase235`.
...
- Adds cache-busted `enterprise_bridge_phase235.js`.
- Keeps public Matrix launch page untouched.

## Test
Open `/game/?v=phase235-bootcache`, then verify the HUD says `PHASE-235-VR-INPUT-SPAWN-CLEAR-LOCK` and the game does not remain stuck on Booting.

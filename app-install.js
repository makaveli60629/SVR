(() => {
  const BUILD = 'PHASE-110B-MODULAR-SVR-APP-DOWNLOAD-MANAGER';
  ...
  function ensureHeadLinks(){
    if(!document.querySelector('link[rel="manifest"]')){const manifest=document.createElement('link');manifest.rel='manifest';manifest.href=appRoot('/manifest.webmanifest?v=phase110b-download');document.head.appendChild(manifest);}
  ...
  async function discoverApk(){
    for(const path of APK_CANDIDATES){try{const response=await fetch(path,{method:'HEAD',cache:'no-store'});if(response.ok){discoveredApk=path;break;}}catch{}}
  ...
  function publicApi(){return{build:BUILD,active:true,modular:true,siteReady:true,gameReady:true,standalone:isStandalone(),platform:platformName(),apkPath:discoveredApk,open:(source='manual')=>{showInstallModal(`SVR download options opened from ${source}.`);setStatus('Download options opened.');},install:()=>installApp(null,true),routes:ROUTES,checkedAt:new Date().toISOString()};}
  ...
  async function boot(){ensureHeadLinks();ensureInstallStyles();await registerServiceWorker();await discoverApk();wireButtons();setTimeout(wireButtons,750);setTimeout(wireButtons,1800);if(new URLSearchParams(location.search).has('install'))setTimeout(()=>showInstallModal(),400);window.SVR_APP_DOWNLOAD_MANAGER=publicApi();}

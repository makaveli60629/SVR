...
function status(message, type = '') {
  ...
  element.className = `avatar-status ${type}`;
}
...
function itemOwned(item, profile) {
  if (item.ownedByDefault || item.bodyTint || item.primary) return true;
...
function optionButton(category, item, profile) {
  const selected = state.outfit?.[category] === item.id;
...
function renderCategory(category) {
  ...
  const items = category === 'palette' ? state.catalog.palettes : state.catalog.categories[category];
  items.forEach((item) => panel.appendChild(optionButton(category, item, profile)));
}
function renderAll({ preserveSaved = false } = {}) {
  ['palette', 'headwear', 'eyewear', 'top', 'shoes', 'accessory'].forEach(renderCategory);
  const profile = account.snapshot().profile;
  ...
  $('#avatarMode').classList.toggle('demo', state.accountMode !== 'api');
...
function select(category, id) {
  ...
  renderAll();
  status('Preview updated. Save the outfit to use it on your profile.', 'info');
}
...
async function save() {
  ...
  try {
    const result = await account.updateProfile({ avatarUrl: MODEL_URL, equippedOutfit: state.outfit });
...
function downloadPortrait() {
  ...
  } catch (error) { status(String(error?.message || error), 'error'); }
}
function bind() {
  document.addEventListener('click', (event) => {
    const option = event.target.closest('.avatar-option');
    if (option) select(option.dataset.category, option.dataset.itemId);
  ...
  $('#resetCamera').addEventListener('click', () => state.viewer.resetView());
...
function qa() {
  ...
  const result = {
    ...
    accountMode: accountState.mode,
    profileAvailable: Boolean(accountState.profile),
    outfitSchema: state.outfit?.schemaVersion || null,
    categories: Object.fromEntries(['palette', 'headwear', 'eyewear', 'top', 'shoes', 'accessory'].map((key) => [key, $$(`[data-category-panel="${key}"] .avatar-option`).length])),
    viewer,
    saved: state.saved,
    lastError: state.lastError,
    checkedAt: new Date().toISOString()
  };
  result.pass = result.ready && result.catalogLoaded && result.profileAvailable && viewer?.modelLoaded && Object.values(result.categories).every((count) => count > 0);
  window.SVR_PHASE346_QA_STATE = result;
  return result;
}
async function boot() {
  try {
    ...
    if (!accountState.profile) {
      location.replace(`/site/login.html?next=${encodeURIComponent(location.pathname)}`);
      return;
    }
    ...
    await state.viewer.loadModel(accountState.profile.avatarUrl || MODEL_URL, 1.72);
    bind();
    renderAll();
    ...
    state.ready = true;
    status(state.viewer.fallbackUsed ? 'The GLB could not load, so the safe mannequin is active.' : 'Avatar loaded. Select equipment and save your outfit.', state.viewer.fallbackUsed ? 'info' : 'ok');
    window.SVR_PHASE346_AVATAR_QA = qa;
    window.SVR_PHASE346_APPLY_PRESET = applyPreset;
    window.SVR_PHASE346_SAVE_OUTFIT = save;
    window.SVR_PHASE346_AVATAR_STATE = state;
  } catch (error) {
    state.lastError = String(error?.message || error);
    status(state.lastError, 'error');
    window.SVR_PHASE346_AVATAR_STATE = state;

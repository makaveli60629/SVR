...
$Root = Get-Location
Write-Host "SVR Phase $Phase starting in $Root" -ForegroundColor Cyan

New-Item -ItemType Directory -Force -Path ".\js\scarlett1" | Out-Null
New-Item -ItemType Directory -Force -Path ".\assets\textures" | Out-Null
...
<head>
  <meta charset="utf-8">
  <title>scarlett VR Phase $Phase</title>
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <style>
    html, body { margin:0; width:100%; height:100%; overflow:hidden; background:#070012; color:white; font-family:Arial; }
    #hud { position:fixed; top:16px; left:16px; z-index:5; background:rgba(10,0,25,.82); padding:16px; border:1px solid #d952ff; border-radius:16px; }
    button { margin:4px; padding:10px 12px; border-radius:10px; border:1px solid #d952ff; background:#5f1fff; color:white; font-weight:bold; }
  </style>
...
<body>
  <div id="hud">
    <h2>scarlett VR Phase $Phase</h2>
    <p>Start Scene: Lobby | VR Ready | Hands Only</p>
    <button data-route="Lobby">Lobby</button>
    <button data-route="Seat">Seat / Poker</button>
    <button data-route="Reiki">Reiki Room</button>
    <button data-route="PGA">PGA Drive</button>
    <button data-route="ChipPutt">Chip/Putt</button>
    <button data-route="VRStore">VR Store</button>
    <button data-route="Sponsor">Sponsor</button>
    <button data-route="Scorpion">Scorpion</button>
    <div id="status">Booting Lobby...</div>
  </div>
...
const VR_READY = $($VRReady.ToString().ToLower());
const POKER_SEATS = 6;
...
function buildLobby() {
  ...
  makeText('LOBBY START HUB', 0, 2.4, -3);
  makeText('VR READY | HANDS ONLY | ROUTE FROM HERE', 0, 1.5, -3);
...
function buildModule(route) {
  rig.position.set(0,0,0);
  makeText(route.toUpperCase() + ' MODULE', 0, 2.4, -3);
...
@"
export const SCARLETT_PHASE = $Phase;
export const START_SCENE = "Lobby";
export const VR_READY = true;
export const POKER_SEATS = 6;
"@ | Set-Content -Path ".\world.js" -Encoding UTF8

@"
export const HANDS_ONLY = true;
export const CONTROLLERS_VISIBLE = false;
"@ | Set-Content -Path ".\control.js" -Encoding UTF8
...
Hands only: true
Poker seats: 6
"@ | Set-Content -Path ".\PHASE-$Phase-REPORT.txt" -Encoding UTF8

Write-Host ""
Write-Host "PASS: Phase $Phase files created." -ForegroundColor Green
Write-Host "PASS: index.html created." -ForegroundColor Green
Write-Host "PASS: index.js created." -ForegroundColor Green
Write-Host "PASS: world.js created." -ForegroundColor Green
Write-Host "PASS: control.js created." -ForegroundColor Green
Write-Host ""

/**
 * MODULE: world.js
 * Builds: stadium deck hole + pit + double walls + rails + stairs + balcony + neon table
 * No external imports (safe).
 */
export async function init(ctx){
  ...
  // Better chair silhouette (seat + back + stem + base ring)
  ...
  // Placeholder players (so you can watch “game area”)
  const players = new THREE.Group(); players.name="Players";
  ...
  for (let i=0;i<8;i++){
    ...
    m.position.set(Math.cos(a)*8.05, pitY+0.45, Math.sin(a)*8.05);
  ...
  const g = new THREE.Mesh(new THREE.CapsuleGeometry(0.26,0.95,6,12),
    new THREE.MeshStandardMaterial({ color:0x111111, emissive:0x8a2be2, emissiveIntensity:0.25 }));
  g.position.set(Math.cos(entranceAngle)*(holeR+0.55), 0.6, Math.sin(entranceAngle)*(holeR+0.55));
  g.rotation.y = -entranceAngle + Math.PI;
  g.name="GUARD_PLACEHOLDER";
  scene.add(g);

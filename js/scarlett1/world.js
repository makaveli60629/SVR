...
function makeGlassBox(w,h,d){
  ...
  const m = new THREE.MeshPhysicalMaterial({
    color: 0x88ccff, roughness: 0.08, metalness: 0.0,
    transmission: 0.85, thickness: 0.08, transparent: true, opacity: 0.35,
  });
...
export async function initWorld(ctx){
  ...
  // walls + ceiling
  const walls = new T.Mesh(new T.CylinderGeometry(wallR, wallR, 9.0, 140, 1, true), matWall);
  walls.position.y = 4.4; scene.add(walls);
  const ceiling = new T.Mesh(new T.CylinderGeometry(wallR, wallR, 0.25, 140),
    new T.MeshStandardMaterial({ color: 0x06070b, roughness: 0.95 })
  );
  ceiling.position.y = 9.0; scene.add(ceiling);

  // pit wall + bottom
  const pitWall = new T.Mesh(new T.CylinderGeometry(pitR, pitR, pitDepth, 120, 1, true), matWall);
  ...
  async function spawnGuard(){
    ...
    try{
      const gltf = await loader.loadAsync(AVATARS.combat);
      const root = gltf.scene || gltf.scenes?.[0];
      if (!root) throw new Error('no scene');
      addSoftEmissive(root);
      ...
      cap.position.set(0, 1.0, 0); guardGroup.add(cap);

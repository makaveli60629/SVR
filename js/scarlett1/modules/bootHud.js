...
export const boot = {
  ...
  mount: async ({ build }) => {
    ...
    wrap.querySelector("#enterVR").onclick = async () => {
      try {
        if (!navigator.xr) return addLine("[xr] navigator.xr not available");
        ...
        addLine(`[xr] start fail: ${e?.message || e}`);
      }
    };
    ...
    wrap.querySelector("#musicOn").onclick = () =>
      window.dispatchEvent(new CustomEvent("scarlett:musicOn"));
    wrap.querySelector("#musicOff").onclick = () =>
      window.dispatchEvent(new CustomEvent("scarlett:musicOff"));
    ...
    return { renderer, scene, camera, rig, addLine };
  },

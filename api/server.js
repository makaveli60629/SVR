...
app.use(cors({
  origin(origin, callback) {
    ...
    const allowed = String(ALLOWED_ORIGIN)
      .split(",")
...
app.post("/api/admin/login", async (req, res) => {
  ...
  if (!ADMIN_EMAIL || !ADMIN_PASSWORD || JWT_SECRET === "CHANGE_ME_DEV_ONLY") {
  ...
  try {
    ...
    return res.status(500).json({
      ok: false,
      error: "Database write failed during login.",
...
app.post("/api/admin/online", requireAdmin, async (req, res) => {
  ...
  try {
    ...
    await dbQuery(
      ...
      [
        isOnline,
        isOnline ? "Admin Online" : "Admin Offline",
        req.admin.email
      ]
    );
    ...
    await writeAdminLog(
      req.admin.email,
      isOnline ? "admin_online" : "admin_offline",
      { source: "api" }
    );
...
app.get("/api/admin/status", async (req, res) => {
  try {
    ...
    const result = await dbQuery(`
      SELECT id, is_online, status_text, updated_by, updated_at
      FROM admin_status
      WHERE id = 1
      LIMIT 1
    `);
...
app.get("/api/messages/admin", requireAdmin, async (req, res) => {
  try {
    ...
    const result = await dbQuery(`
      SELECT id, name, email, subject, message, source, is_read, created_at
      FROM site_messages
...
app.listen(PORT, () => {
  console.log(`SVR AWS PostgreSQL API listening on port ${PORT}`);
});

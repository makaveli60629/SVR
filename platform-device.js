...
(()=>{
  ...
  const MEDIA_FIT_BUILD='PHASE-417-ANDROID-VIEWPORT-MEDIA-FIT-LOCK';
  ...
  const state={build:BUILD,mediaFitBuild:MEDIA_FIT_BUILD,device,browser,quest:questUa,android,ios,mobile,touch,ipadDesktop,standalone:Boolean(navigator.standalone||matchMedia?.('(display-mode: standalone)')?.matches),vrCapable:false,orientation:null,viewport:null,screen:null,mediaFitLoaded:false,checkedAt:null};
  ...
  function installMobileMediaFit(){
    if(!mobile||questUa||!location.pathname.startsWith('/game/'))return false;
    const id='svr-phase417-mobile-media-fit';
    if(document.getElementById(id)){state.mediaFitLoaded=true;return true}
    const link=document.createElement('link');link.id=id;link.rel='stylesheet';link.href='/game/styles/phase417_android_viewport_media_fit.css?v=phase417';link.dataset.build=MEDIA_FIT_BUILD;link.onload=()=>{state.mediaFitLoaded=true;root.classList.add('svr-media-fit-ready')};link.onerror=()=>{state.mediaFitLoaded=false};document.head?.appendChild(link);return true
  }
  function publish(){viewport();classes();installMobileMediaFit();state.checkedAt=new Date().toISOString();window.SVR_PLATFORM_DEVICE={...state};window.dispatchEvent(new CustomEvent('svr:platform-device',{detail:{...state}}));return window.SVR_PLATFORM_DEVICE}
  ...
  window.SVR_PLATFORM_DEVICE_QA=()=>({...publish(),route:route(),mediaFitBuild:MEDIA_FIT_BUILD,mediaFitRequested:Boolean(mobile&&!questUa&&location.pathname.startsWith('/game/')),mediaFitLoaded:state.mediaFitLoaded,pass:Boolean(state.device&&state.browser&&state.viewport?.width&&state.viewport?.height)});window.SVR_PLATFORM_GAME_ROUTE=route;
})();